...
SOURCE_SHA256 = "2e43ae742040abdda53f1c32fd80668c6082bb5eb07f4663218fc2b93d980fd3"
DEFAULT_OUTPUT = ROOT / (
...
def build_register() -> dict[str, Any]:
    if (
        ...
        raise remediation.RemediationError("candidate.4.10 AI review receipt drift")
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    if source.get("status") != (
        "AI_AUDIT_REVISE_POSTHOC_ATTESTATION_FORGEABLE_FAIL_CLOSED"
    ):
        raise remediation.RemediationError("candidate.4.10 fail-closed status drift")
    review = source.get("review_result")
    if not isinstance(review, dict) or not isinstance(review.get("findings"), list):
        raise remediation.RemediationError("candidate.4.10 finding register absent")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Materialize candidate.4.10 AI finding semantics"
    )
