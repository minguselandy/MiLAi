...
def test_dg24_record_trace_masks_run_local_command_fields_only() -> None:
    baseline = RetrievalTraceCommand(
        request_id="request-a",
        route="L1",
        consistency="CANONICAL_REQUIRED",
        query_fingerprint="query",
        query_plan={"semantic": "fixed"},
        requested_scope={"project_ids": ["p1"]},
        ...
        causal_wait_outcome=None,
        causal_waited_ms=1,
        execution_trace={"elapsed_ms": 4, "decision": "fixed"},
        stage_metrics={"latency_ms": 3},
    )
    traced = replace(
        baseline,
        ...
        duration_ms=500,
        causal_waited_ms=10,
        execution_trace={"elapsed_ms": 400, "decision": "fixed"},
        stage_metrics={"latency_ms": 300},
    )
    changed = replace(
        traced,
        requested_scope={"project_ids": ["p2"]},
    )

    baseline_value = _repository_call_semantics(
        "record_trace", _call_material("record_trace", [baseline], {})
    )
    traced_value = _repository_call_semantics(
        "record_trace", _call_material("record_trace", [traced], {})
    )
    assert baseline_value == traced_value
    assert baseline_value != _repository_call_semantics(
        "record_trace", _call_material("record_trace", [changed], {})
    )
