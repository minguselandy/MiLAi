    ...
    items = [
        ...
        },
        {
            **_item("newer", "2023-09-30T06:40:00+00:00"),
            "payload": {"memory_text": "user: My current total is five tops from H&M."},
        },
    ]
...
def test_single_current_canonical_state_allows_an_unbounded_valid_interval() -> None:
    plan = _plan("What is the current OpenWorker MCP connection status?")
    item = {
        **_item("CONNECTED", "2026-09-02T03:00:00+00:00"),
...
def test_current_state_uses_unique_canonical_address_for_unbounded_candidates() -> None:
    plan = _plan("What is the current release target for orchid-release?")
    target = {
        **_item("AURORA-QUARTZ-8421", "2026-09-02T03:00:00+00:00"),
        ...
        "payload": {
            ...
            "value": "AURORA-QUARTZ-8421",
        },
    }
    decision = {
        **_item("GATE-GREEN-275", "2026-09-02T03:01:00+00:00"),
        "subject_id": "orchid-release",
    ...
    result = execute_query_operator(plan, [decision, target])

    assert result is not None and result["status"] == "OK"
    assert result["value"] == "AURORA-QUARTZ-8421"


def test_current_state_keeps_unbounded_canonical_address_ties_uncertain() -> None:
    plan = _plan("What is the current release status for orchid-release?")
    first = {
        **_item("first", "2026-09-02T03:00:00+00:00"),
        "subject_id": "orchid-release",
        "predicate": "release.alpha",
        "valid_time_from": None,
    }
    second = {
        **_item("second", "2026-09-02T03:01:00+00:00"),
        "subject_id": "orchid-release",
        "predicate": "release.beta",
        "valid_time_from": None,
    }

    result = execute_query_operator(plan, [first, second])
...
def test_latest_user_state_abstains_without_a_user_fact() -> None:
    ...
    assert all(
        requirement.semantic_roles.actor is None
...
def test_relative_point_can_use_a_reranked_temporal_sentence_without_term_overlap() -> None:
    ...
    planting = {
        **_item("planting", "2023-04-21T00:30:00+00:00"),
        "payload": {"memory_text": "user: Today I planted 12 tomato seedlings in the garden."},
    }
    ...
    assert result is not None and result["status"] == "OK"
    assert result["value"] == {
        "selected": "user: Today I planted 12 tomato seedlings in the garden.",
