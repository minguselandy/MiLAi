...
def test_query_planner_emits_versioned_safe_l1_plan_without_raw_query() -> None:
    ...
    trace_payload = json.dumps(payload_free_query_plan(plan), sort_keys=True)
    assert "sensitive" not in trace_payload
    assert "synthetic" not in trace_payload
    assert "memory_query_ir" not in trace_payload
    assert "operator_arguments" not in trace_payload

    future_internal_payload = plan.model_dump(mode="json")
    future_internal_payload["future_query_analysis"] = {
        "surface_terms": ["sensitive", "synthetic"]
    }
    assert "future_query_analysis" not in payload_free_query_plan(
...
def test_binary_event_order_requires_two_confident_typed_anchors() -> None:
    ...
    assert ambiguous.memory_query_ir.mode == "EVIDENCE"
    assert ambiguous.query_task_contract is not None
    assert (
        ambiguous.query_task_contract.parse_disposition
        == ParseDisposition.BEST_EFFORT_RECALL
    )
    assert ambiguous.query_execution_plan is not None
