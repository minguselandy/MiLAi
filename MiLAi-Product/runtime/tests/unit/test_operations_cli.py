...
def test_agent_config_emits_secret_free_codex_http_profile() -> None:
    ...
    assert server == {  # type: ignore[comparison-overlap]
    ...
    assert "${MILAI_AGENT_READER_TOKEN}" not in repr(config)
