...
def test_canonical_context_budget_counts_reader_projection_not_opaque_lineage() -> None:
    canonical = {
        "claim_version_id": "version-1",
        "claim_id": "claim-1",
        "subject_id": "synthetic:mcp-status-demo",
        "predicate": "mcp.connection.status",
        "claim_type": "STATE",
        "payload": {"value": "CONNECTED"},
        "authority": "ACTION_SAFE",
        "evidence_ids": [f"evidence-{index}-" + "x" * 64 for index in range(12)],
        "open_issue_ids": [],
        "relevance_score": 1.0,
    ...
    bounded = _apply_context_budget((accepted, [], [canonical], []), 768, progressive, degraded)

    assert bounded[2] == [canonical]
    assert bounded[0] == accepted
    assert progressive["context_budget_truncated"] is False
    assert progressive["context_token_upper_bound"] <= 768
    assert degraded == set()
