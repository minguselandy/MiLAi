...
def list_proposals():  # type: ignore[no-untyped-def]
    ...
    if project_id is not None and (not project_id.strip() or len(project_id) > 512):
        raise ApiError(code="INVALID_REQUEST", message="Invalid project filter.", status_code=400)
    proposals = _service().list_proposals(_context(), status, limit, project_id=project_id)
