...
@evidence_blueprint.post("/browse")
def browse_evidence():  # type: ignore[no-untyped-def]
    try:
        command = EvidenceBrowseRequest.model_validate(request.get_json(silent=True))
        result = _service().browse(_context(), command)
    except ValidationError as exc:
        raise ApiError(
            code="INVALID_REQUEST", message="Evidence browse arguments are invalid.",
            status_code=400, details=_validation_details(exc),
        ) from exc
    except ValueError as exc:
        raise ApiError(
            code="INVALID_EVIDENCE_CURSOR",
            message="Cursor does not match this request or expired.",
            status_code=400,
        ) from exc
...
def get_evidence(evidence_id: UUID):  # type: ignore[no-untyped-def]
    ...
    if project_id is not None and not 1 <= len(project_id.strip()) <= 512:
        raise ApiError(
            code="INVALID_REQUEST", message="project_id must contain 1..512 characters.",
            status_code=400,
        )
    try:
        view = _service().get(_context(), evidence_id, project_id=project_id)
