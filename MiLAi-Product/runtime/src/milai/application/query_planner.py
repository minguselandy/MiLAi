...
from milai.application.query_task_compiler import project_query_task_contract_v01
from milai.domain.query_execution import QueryExecutionPlanV01
from milai.domain.query_task_contract import ParseDisposition, QueryTaskContractV01
...
class QueryPlanner:
    ...
    def plan(
        ...
        *,
        ...
        query_task_contract: QueryTaskContractV01 | None = None
        query_execution_plan: QueryExecutionPlanV01 | None = None
        if request.route == "L0":
            query_ir = _exact_state_ir(request)
        else:
            query = request.query or ""
            query_task_contract = self._compiler.compile_contract(
                query,
                reference_time=reference_time,
            )
            query_ir = project_query_task_contract_v01(
                query_task_contract,
                query=query,
                scope=request.requested_scope,
            )
            if query_task_contract.parse_disposition != ParseDisposition.UNSUPPORTED:
                query_execution_plan = derive_query_execution_plan_v01(
                    query_task_contract
                )
        ...
        return QueryPlan(
            ...
            query_task_contract=query_task_contract,
...
_PAYLOAD_FREE_QUERY_PLAN_FIELDS = frozenset(
    {
        "planner_version",
        "intent",
        "entities",
...
def payload_free_query_plan(
    plan: QueryPlan | Mapping[str, object],
) -> dict[str, object]:
    """Return the public/trace plan without query-derived lexical material."""
    ...
    # Internal execution views contain raw lexical cues and operand anchors.
    # Keep this trust-boundary projection allowlist-based so a future internal
    # planner field cannot become public or durable without an explicit review.
    return {
        key: value
        for key, value in payload.items()
        if key in _PAYLOAD_FREE_QUERY_PLAN_FIELDS
    }


__all__ = ["QueryPlanner", "payload_free_query_plan"]
