...
def resolve_requirement_state(
    *,
    ...
    # Empty authoritative operands need no diagnostic lookup material.
    span_by_id = {item.span_id: item for item in spans} if bindings else {}
    interpretation_by_id = (
        {item.interpretation_id: item for item in interpretations} if bindings else {}
    )
    binding_rows = sorted(
        bindings,
