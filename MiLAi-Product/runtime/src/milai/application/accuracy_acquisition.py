...
def accuracy_bundle_query_text(
    ...
    cue_by_requirement = {
        cue.requirement_slot: cue
        for cue in query_ir.lexical_cues
        if cue.requirement_slot in targets
    }
    for requirement in query_ir.requirements:
        if requirement.slot_id not in targets:
            continue
        cue = cue_by_requirement.get(requirement.slot_id)
        values = _retrieval_surfaces(requirement, cue)
        for value in values:
...
def execute_requirement_complete_bundle(
    ...
    *,
    ...
    cue_by_requirement = {
        cue.requirement_slot: cue
        for cue in query_ir.lexical_cues
        if cue.requirement_slot in bundle.target_requirement_ids
    }
    for requirement in requirements:
        ranked = _rank_requirement(
            requirement,
            governed,
            lexical_cue=cue_by_requirement.get(requirement.slot_id),
        )
        selected = ranked[:cap]
...
def _rank_requirement(
    ...
    evidence: Sequence[Mapping[str, Any]],
    *,
    lexical_cue: LexicalCueSetV01 | None = None,
) -> list[dict[str, Any]]:
    terms = _terms(_retrieval_surfaces(requirement, lexical_cue))
    ...
    for item in evidence:
        ...
        # Planner-owned retrieval hints are recall/ranking features, not
        # semantic admission rules. Only old IRs without a cue retain the
        # historical hard-anchor behavior for replay compatibility.
        if lexical_cue is None and terms and not _entity_anchor_match(terms, observed):
            continue
        social_relation_match = bool(observed.intersection(_SOCIAL_CIRCLE_TERMS))
        incidental = exhaustive and _INCIDENTAL_EVENT_MENTION.search(content) is not None
        source_derived = (
        ...
        value["accuracy_local_score"] = (
            ...
            + (0.10 if requires_social_circle and social_relation_match else 0.0)
            - (0.20 if incidental else 0.0)
            - (0.20 if source_derived else 0.0)
            + (
                0.10
                if "USER"
                in {
...
def _retrieval_surfaces(
    requirement: EvidenceRequirementV02,
    lexical_cue: LexicalCueSetV01 | None,
) -> list[str]:
    """Use retrieval projections without promoting semantic constraints.

    The presence of a cue means the planner supplied a retrieval projection,
    including an intentionally empty projection. Semantic entity/predicate
    constraints remain Binding inputs. Serialized legacy IRs with no cue keep
    their historical surfaces as a narrow compatibility fallback.
    """

    if lexical_cue is not None:
        return [
            *lexical_cue.surface_terms,
            *lexical_cue.morphological_variants,
            *lexical_cue.entity_aliases,
            *lexical_cue.relation_cues,
        ]
