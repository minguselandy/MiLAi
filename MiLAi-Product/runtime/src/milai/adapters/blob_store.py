...
def _timing(name: str) -> AbstractContextManager[None]:
...
class LocalContentAddressedBlobStore:
    ...
    def _write_locked(
        ...
        if target.exists():
            ...
            # A previous attempt may have renamed the file then failed to sync
            # its directory. Existence alone is not a durable-write receipt.
            with target.open("rb") as handle, _timing("blob_file_sync_ms"):
                os.fsync(handle.fileno())
            self._sync_blob_directories(target)
        ...
        try:
            ...
            with os.fdopen(file_descriptor, "wb", closefd=True) as handle:
                ...
                handle.flush()
                with _timing("blob_file_sync_ms"):
                    os.fsync(handle.fileno())
    ...
    def _sync_blob_directories(self, target: Path) -> None:
        # Publish leaf -> tenant -> root -> provisioned parent. Sync even when
        # a directory already exists: another writer may have just created it,
        # or an earlier attempt may have failed before reaching this boundary.
        directory = target.parent
        while True:
            descriptor = os.open(directory, os.O_RDONLY | os.O_DIRECTORY)
            try:
                with _timing("blob_directory_sync_ms"):
                    os.fsync(descriptor)
            finally:
                os.close(descriptor)
            if directory == self._root.parent:
                break
    ...
    def _blob_lock(self, tenant_id: UUID, content_hash: str) -> Iterator[None]:
        ...
        try:
            ...
            with _timing("blob_lock_wait_ms"):
