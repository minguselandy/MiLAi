...
_SEMANTIC_OPERATOR_FAMILIES = frozenset(
    {
        "LOOKUP",
        "TEMPORAL_FILTER",
        "TEMPORAL_ORDER",
...
class MemoryQueryIRV02(BaseModel):
    ...
    def validate_executable_shape(self) -> Self:
        ...
        requirement_ids = [item.slot_id for item in self.requirements]
        if len(requirement_ids) != len(set(requirement_ids)):
            raise ValueError("MemoryQueryIR requirement identities must be unique")
        ...
        if operator_family is not None and operator_family not in _SEMANTIC_OPERATOR_FAMILIES:
            raise ValueError("MemoryQueryIR carries an unsupported operator family")
        produced: set[str] = set()
        for index, step in enumerate(self.steps):
            if len(step.inputs) != len(set(step.inputs)) or len(step.outputs) != len(
                set(step.outputs)
            ):
                raise ValueError("MemoryQueryIR step inputs and outputs must be unique")
            missing_inputs = set(step.inputs).difference(produced)
            if missing_inputs:
                raise ValueError(
                    f"MemoryQueryIR step {index} reads values without a producer: "
                    f"{sorted(missing_inputs)}"
                )
            duplicate_outputs = set(step.outputs).intersection(produced)
            if duplicate_outputs:
                raise ValueError(
                    "MemoryQueryIR step outputs must have one producer: "
                    f"{sorted(duplicate_outputs)}"
                )
            produced.update(step.outputs)
        if operator_family in {"COUNT", "SUM", "DIVIDE", "TEMPORAL_DISTANCE"} and (
            self.answer_shape != "SCALAR"
        ):
            raise ValueError(f"{operator_family} requires a scalar answer shape")
        target_event = next(
            (
                requirement
        ...
        if operator_family in {"TEMPORAL_ORDER", "TEMPORAL_DISTANCE"}:
            ...
            if operator_family == "TEMPORAL_ORDER" and len(required_events) < 2:
                raise ValueError(f"{operator_family} requires at least two typed event bindings")
            if operator_family == "TEMPORAL_DISTANCE":
                _validate_temporal_distance_step_operands(self.steps, required_events)
        return self


def _validate_temporal_distance_step_operands(
    steps: list[MemoryQueryStep],
    required_events: list[EvidenceRequirementV02],
) -> None:
    reduction = next(
        (
            step
    ...
    raw_operands = (
        reduction.constraints.get("operator_operands") if reduction is not None else None
    )
    if not isinstance(raw_operands, list) or len(raw_operands) != 2:
        raise ValueError("TEMPORAL_DISTANCE requires two explicit operator operands")
    operands = [item for item in raw_operands if isinstance(item, dict)]
    if len(operands) != 2:
        raise ValueError("TEMPORAL_DISTANCE operator operands must be structured")
    binding_slots = {
        item.get("slot_id")
        for item in operands
        if item.get("kind") == "REQUIREMENT_BINDING"
        and isinstance(item.get("slot_id"), str)
    }
    reference_operands = [
        item for item in operands if item.get("kind") == "QUERY_REFERENCE_TIME"
    ]
    event_slots = {item.slot_id for item in required_events}
    if binding_slots != event_slots:
        raise ValueError("TEMPORAL_DISTANCE operands must bind every required event")
    if len(event_slots) == 2 and reference_operands:
        raise ValueError("two-event TEMPORAL_DISTANCE cannot add a runtime reference operand")
    if len(event_slots) == 1 and len(reference_operands) != 1:
        raise ValueError("one-event TEMPORAL_DISTANCE requires the query reference time")
    if len(event_slots) not in {1, 2}:
        raise ValueError("TEMPORAL_DISTANCE requires one or two event bindings")
    for item in reference_operands:
        raw_value = item.get("value")
        timezone = item.get("timezone")
        if not isinstance(raw_value, str) or not isinstance(timezone, str) or not timezone:
            raise ValueError("query-reference-time operand requires value and timezone")
        try:
            value = datetime.fromisoformat(raw_value)
        except ValueError as exc:
            raise ValueError("query-reference-time operand value is invalid") from exc
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("query-reference-time operand value must be timezone-aware")
