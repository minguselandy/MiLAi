...
class RetrievalRepository:
    ...
    def search_evidence_within_sources(
        ...
        *,
        ...
        """Search exact turns only inside previously acquired source/session pairs."""

        query_terms = evidence_query_terms(query)
        unique_evidence_ids = list(dict.fromkeys(coarse_evidence_ids))
        if not query_terms or not unique_evidence_ids:
            return []
        if len(unique_evidence_ids) > 120:
            raise ValueError("intra-source acquisition supports at most 120 coarse anchors")
        if not 1 <= hits_per_session <= 8:
            raise ValueError("intra-source hits_per_session must be between 1 and 8")
        if not 1 <= max_items <= 120:
            raise ValueError("intra-source max_items must be between 1 and 120")
        tsquery = " | ".join(query_terms)
        identifiers = [UUID(value) for value in unique_evidence_ids]
        try:
            with self._database.connection(
                context,
                read_only=True,
                ...
                row = connection.execute(
                    "SELECT milai.search_evidence_projection_within_sources("
                    "%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (
                        context.tenant_id,
                        context.actor_id,
            ...
            raise RuntimeError("intra-source Evidence projection is unavailable") from exc
        if row is None:
            raise RuntimeError("intra-source Evidence search returned no result")
        return _array_of_objects(
            row[0],
            "search_evidence_projection_within_sources",
        )
    ...
    def hydrate_evidence_by_ids(
        ...
        *,
        ...
        with self._database.connection(
            ...
            row = connection.execute(
                "SELECT milai.hydrate_evidence_projection_by_id_v2("
                "%s, %s, %s, %s, %s, %s, %s)",
                (
                    context.tenant_id,
                    context.actor_id,
