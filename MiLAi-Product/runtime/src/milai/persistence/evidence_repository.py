...
from milai.domain.evidence import EvidenceBrowseRequest
...
class EvidenceRepository:
    ...
    def browse(
        ...
        *, snapshot: str | None, after_time: str | None, after_id: str | None,
    ) -> tuple[list[dict[str, Any]], str]:
        with self._database.connection(context, read_only=True) as connection:
            if snapshot is None:
                stamp = connection.execute("SELECT clock_timestamp()").fetchone()
                assert stamp is not None
                snapshot = stamp[0].isoformat()
            rows = connection.execute(
                """
                SELECT e.evidence_id, e.source_type, e.subject_id, e.observed_at, e.captured_at
                FROM milai.evidence_record e
                WHERE e.tenant_id = %s
                  AND e.revoked_at IS NULL AND e.retention_state = 'READABLE'
                  AND e.permission_snapshot @> %s::jsonb
                  AND (%s::text IS NULL OR e.source_type = %s)
                  AND (%s::text IS NULL OR e.subject_id = %s)
                  AND e.captured_at <= %s::timestamptz
                  AND (%s::timestamptz IS NULL OR (e.captured_at, e.evidence_id)
                       > (%s::timestamptz, %s::uuid))
                ORDER BY e.captured_at, e.evidence_id
                LIMIT %s
                """,
                (context.tenant_id, Jsonb({"readable": True, "project_ids": [request.project_id]}),
                 request.source_type, request.source_type, request.subject_id, request.subject_id,
                 snapshot, after_time, after_time, after_id, request.limit + 1),
            ).fetchall()
        return [{
            "evidence_id": str(row[0]), "source_type": row[1], "subject_id": row[2],
            "observed_at": row[3].isoformat(), "captured_at": row[4].isoformat(),
