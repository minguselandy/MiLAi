...
from milai.adapters.http_models import validate_model_url
...
class ApiSettings(CommonSettings):
    ...
    embedding_model_revision: str | None = Field(default=None, min_length=1, max_length=255)
    embedding_http_timeout_seconds: float = Field(default=10.0, gt=0, le=60)
    ...
    retrieval_query_preserving_union_enabled: bool = False
    retrieval_additive_union_v0_2_enabled: bool = False
    retrieval_evidence_set_selection_enabled: bool = False
    retrieval_continuation_v0_1_enabled: bool = False
    intra_source_acquisition_v0_1_mode: Literal["OFF", "SHADOW"] = "OFF"
    reader_informational_soft_admission_v0_2_enabled: bool = False
    reader_instance_preserving_admission_v0_1_enabled: bool = False
    budget_invariant_context_v0_1: bool = False
    feature_profile: Literal["BASELINE", "FORMED_SHADOW", "FORMED_CANARY"] = "BASELINE"
    reader_evidence_policy_v0_1: ReaderEvidencePolicy = (
    ...
    retrieval_reranker_provider: Literal["none", "onnx_cross_encoder", "http_reranker"] = "none"
    retrieval_reranker_endpoint: str | None = None
    retrieval_reranker_http_timeout_seconds: float = Field(default=10.0, gt=0, le=60)
    retrieval_reranker_max_concurrency: int = Field(default=4, ge=1, le=64)
    ...
    database_pool_max_waiting: int = Field(default=32, ge=1, le=1024)
    database_pool_max_per_tenant: int | None = Field(default=None, ge=1, le=64)
    ...
    def validate_pool_bounds(self) -> ApiSettings:
        ...
        if (self.database_pool_max_per_tenant is not None
                and self.database_pool_max_per_tenant > self.database_pool_max_size):
            raise ValueError("database_pool_max_per_tenant cannot exceed database_pool_max_size")
        ...
        elif self.embedding_provider == "http_embeddings":
            if self.embedding_endpoint is None or self.embedding_model_revision is None:
                raise ValueError("http_embeddings requires endpoint and explicit model revision")
            validate_model_url(self.embedding_endpoint)
        ...
        elif self.retrieval_reranker_provider == "http_reranker":
            if (self.retrieval_reranker_endpoint is None
                    or self.retrieval_reranker_revision == "disabled"):
                raise ValueError("http_reranker requires endpoint and explicit model revision")
            validate_model_url(self.retrieval_reranker_endpoint)
        elif (
        ...
        if (
            self.retrieval_additive_union_v0_2_enabled
            and self.retrieval_query_preserving_union_enabled
        ):
            raise ValueError("query-preserving union V1 and additive union V2 are exclusive")
        if (
            self.retrieval_additive_union_v0_2_enabled
            and self.retrieval_type_directed_acquisition_enabled
        ):
            raise ValueError("additive union V2 excludes lexical enrichment")
        if self.retrieval_evidence_set_selection_enabled and (
            self.retrieval_additive_union_v0_2_enabled
            or self.reader_informational_soft_admission_v0_2_enabled
        ):
            raise ValueError("Product-08 candidate paths exclude EvidenceSet selection")
        return self
    ...
    def safe_summary(self) -> dict[str, object]:
        return {
            ...
            "embedding_source_dimensions": self.embedding_source_dimensions,
            ...
            "retrieval_query_preserving_union_enabled": (
                self.retrieval_query_preserving_union_enabled
            ),
            "retrieval_additive_union_v0_2_enabled": (
                self.retrieval_additive_union_v0_2_enabled
            ),
            "retrieval_evidence_set_selection_enabled": (
                self.retrieval_evidence_set_selection_enabled
            ),
            "retrieval_continuation_v0_1_enabled": (
                self.retrieval_continuation_v0_1_enabled
            ),
            "intra_source_acquisition_v0_1_mode": (
                self.intra_source_acquisition_v0_1_mode
            ),
            "reader_informational_soft_admission_v0_2_enabled": (
                self.reader_informational_soft_admission_v0_2_enabled
            ),
            "reader_instance_preserving_admission_v0_1_enabled": (
                self.reader_instance_preserving_admission_v0_1_enabled
            ),
            "budget_invariant_context_v0_1": self.budget_invariant_context_v0_1,
            "feature_profile": self.feature_profile,
            "reader_evidence_policy_v0_1": self.reader_evidence_policy_v0_1.value,
    ...
    @property
    def progressive_context_evidence_v0_1(self) -> bool:
        """Read-only compatibility view; configuration uses reader_evidence_policy_v0_1."""

        return (
            self.reader_evidence_policy_v0_1
...
def load_settings(environ: Mapping[str, str] | None = None) -> RuntimeSettings:
    ...
    legacy_progressive = source.get("MILAI_PROGRESSIVE_CONTEXT_EVIDENCE_V0_1")
    if legacy_progressive is not None:
        legacy_policy = {
            "true": ReaderEvidencePolicy.GOVERNANCE_ADMITTED_SOFT_RANKED.value,
            "false": ReaderEvidencePolicy.DECISION_ACCEPTED_ONLY.value,
        }.get(legacy_progressive.casefold())
        if legacy_policy is None:
            raise SettingsError(
                "invalid MiLAi configuration field: progressive_context_evidence_v0_1"
            )
        declared_policy = values.get("reader_evidence_policy_v0_1")
        if declared_policy is not None and declared_policy != legacy_policy:
            raise SettingsError(
                "reader_evidence_policy_v0_1 conflicts with "
                "progressive_context_evidence_v0_1"
            )
        values["reader_evidence_policy_v0_1"] = legacy_policy
    try:
