...
Agent Integration v0.1 remains a local candidate, Schema 0.1.x is not frozen,
and real personal data is still denied until an encrypted backup/key-recovery drill is independently accepted. The
current migration head is `0055_working_state_ref_capacity`; migration `0027_embedding_identity` binds vector rows
...
`milai-openworker-mcp`, `milai-langgraph`, `milai-autogen`, and `milai-hooks`. Runtime never imports them. MCP supports
stdio and authenticated Streamable HTTP profiles, including the single-endpoint `codex-full` lifecycle. Legacy
reader/submitter/reviewer/operator allowlists remain available. The separated reviewer profile uses a distinct
