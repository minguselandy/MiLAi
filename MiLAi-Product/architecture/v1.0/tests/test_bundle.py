...
class ArchitectureBundleTest(unittest.TestCase):
    ...
    def test_frozen_release_requires_independent_acceptance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            copied = Path(temporary) / "bundle"
            shutil.copytree(BUNDLE_ROOT, copied)
            ...
            errors = collect_errors(copied, PROJECT_ROOT)
            self.assertIn(
                "FREEZE_REVIEW acceptance missing: Decision：`ACCEPT`",
                errors,
            )

    def test_manifest_rejects_forged_acceptance_binding(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            copied = Path(temporary) / "bundle"
            shutil.copytree(BUNDLE_ROOT, copied)
            manifest_path = copied / "architecture_manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["review"]["sha256"] = "0" * 64
    ...
    def test_promotion_record_cannot_drop_independent_acceptance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            copied = Path(temporary) / "bundle"
            shutil.copytree(BUNDLE_ROOT, copied)
            ...
            errors = collect_errors(copied, PROJECT_ROOT)
            self.assertIn(
                "AUTHOR_PREFLIGHT promotion boundary missing: Independent decision: ACCEPT",
                errors,
            )
    ...
    def test_release_mode_requires_external_manifest_trust_anchor(self) -> None:
        errors = collect_hash_errors(
            BUNDLE_ROOT,
            PROJECT_ROOT,
            WORKSPACE_ROOT,
        ...
        self.assertEqual(
            errors,
            ["release verification requires an external expected manifest SHA-256"],
        )

    def test_release_mode_rejects_external_manifest_digest_mismatch(self) -> None:
        errors = collect_hash_errors(
            BUNDLE_ROOT,
            PROJECT_ROOT,
            WORKSPACE_ROOT,
