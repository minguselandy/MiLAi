...
async def _call_agent_memory(parameters: StdioServerParameters) -> dict[str, Any]:
    async with Client(stdio_client(parameters), mode="2026-07-28") as client:
        tools = await client.list_tools()
...
def test_agent_memory_stdio_returns_small_host_reasoning_contract() -> None:
    _RuntimeHandler.requests = []
    runtime = ThreadingHTTPServer(("127.0.0.1", 0), _RuntimeHandler)
    thread = threading.Thread(target=runtime.serve_forever, daemon=True)
    thread.start()
    try:
        ...
        result = asyncio.run(_call_agent_memory(parameters))
    finally:
        runtime.shutdown()
        runtime.server_close()
        thread.join(timeout=5)
    ...
    assert result["snapshot"] == {"canonical_position": 7}
    assert result["continuation"] is None
    assert result["profile"] == {
        "requested": "MCP_INTERACTIVE_STANDARD_V01",
    ...
    assert result["evidence"] == [
        {
            ...
            "text": '{"value": "synthetic persisted state"}',
            "evidence_ids": ["33333333-3333-4333-8333-333333333333"],
            "source": {
    ...
    assert "access_trace" not in result
    assert "derived_result" not in result
