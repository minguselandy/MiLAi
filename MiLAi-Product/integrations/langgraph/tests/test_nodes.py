...
def test_capture_node_propagates_only_explicit_source_identity() -> None:
    calls: list[dict[str, Any]] = []

    class _CaptureMemory:
        def after_user_observation(self, **options: Any) -> str:
            calls.append(options)
            return "CAPTURED"

    node = capture_node(_CaptureMemory())  # type: ignore[arg-type]
    result = node(
        {
            "verified_user_observation": "synthetic",
    ...
    assert result == {"milai_capture": "CAPTURED"}
    assert calls[0]["source_context"]["turn_ordinal"] == 4

    node(
        {
            "verified_user_observation": "legacy synthetic",
    ...
    assert calls[1]["source_context"] is None
