...
class HttpxAsyncTransport:
    ...
    async def send(
        ...
        started = monotonic() if self._timing_enabled else 0.0
        ...
        async def trace(event: str, info: dict[str, Any]) -> None:
            nonlocal dropped
            stage, _, ending = event.rpartition(".")
            if stage not in {
            ...
            } or ending not in {"started", "complete", "failed"}:
                return
            # info includes headers, streams and exception messages: never retain it.
            if len(events) < 32:
                events.append({"event": event, "monotonic_s": monotonic()})
            else:
                dropped += 1
        ...
        try:
            response = await self._client.request(
                ...
                extensions={"trace": trace} if self._timing_enabled else None,
            )
            ...
            if self._timing_enabled:
                ...
                logging.getLogger(__name__).info("MILAI_HTTP_TRANSPORT_TIMING %s", json.dumps({
                    "schema_version": "http-transport-timing-v1",
                    ...
                    "transport_ms": (finished - started) * 1000,
                    "status_code": response.status_code if response is not None else None,
                    "trace_events": events, "dropped_events": dropped,
                }, sort_keys=True))
...
class AsyncMilaiClient:
        ...
        **options: Any,
        ...
        body = (
            await self._request(
                ...
                read_only_retry_safe=True,
                extra_headers=(
                    {
                        "X-MiLA-Host-Principal-Binding-Digest": (
                            host_principal_binding_digest
                        )
                    }
                    if host_principal_binding_digest is not None
                    else None
                ),
            )
        ).body
    ...
    async def get_working_state(self, binding: Mapping[str, Any]) -> dict[str, Any]:
        """Read the current Host-owned working state for a server-bound scope."""
    ...
    async def update_working_state(
        ...
        *,
        ...
        """Append a full Host working-state version using exact CAS."""
    ...
    async def append_host_execution_event(
        self,
        payload: Mapping[str, Any],
        *,
        operation_id: str,
    ) -> dict[str, Any]:
        """Append one trusted Host observation to the shadow Event journal."""
    ...
    async def get_host_execution_event_window(
        self,
        binding: Mapping[str, Any],
    ) -> dict[str, Any]:
        """Read one exact task-bound shadow Event window."""
    ...
    async def write_note(
        ...
        """Commit one Host Note change using a caller-retained operation identifier."""
        await self._require_feature("host_notes_v1")
        return (await self._request(
            "POST", "/v1/notes/write", dict(payload), operation_id=operation_id, no_retry=True,
        )).body

    async def get_note(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        await self._require_feature("host_notes_v1")
        return (await self._request("POST", "/v1/notes/get", dict(payload))).body

    async def browse_notes(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        await self._require_feature("host_notes_v1")
        return (await self._request("POST", "/v1/notes/browse", dict(payload))).body

    async def get_note_operation(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        await self._require_feature("host_notes_v1")
        return (await self._request("POST", "/v1/notes/operations/get", dict(payload))).body

    async def browse_evidence(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        """List currently readable Evidence metadata in an explicitly narrowed project."""
        await self._require_feature("evidence_project_browse")
        return (await self._request("POST", "/v1/evidence/browse", dict(payload))).body
    ...
    async def get_evidence_content(
        self, evidence_id: str, *, project_id: str,
    ) -> dict[str, Any]:
        """Read source content after Runtime checks the explicitly narrowed project.

        Unlike get_evidence_metadata, this method intentionally returns content.
        A revoked/unreadable source has content=null, never an empty source string.
        The private Host must supply its authenticated project, not model input.
        """
        if not project_id or not project_id.strip() or len(project_id) > 512:
            raise ValueError("project_id must contain 1..512 characters")
        await self._require_feature("evidence_project_read")
        path = f"/v1/evidence/{UUID(evidence_id)}?" + urlencode({"project_id": project_id})
    ...
    async def list_proposals(
        ...
        *,
        ...
        body = (await self._request("GET", "/v1/proposals?" + urlencode(query))).body
    ...
    async def get_proposal(self, proposal_id: str) -> dict[str, Any]:
        return (await self._request("GET", f"/v1/proposals/{proposal_id}")).body
    ...
    async def namespace_cleanup_status(
        ...
        *,
        ...
        return (await self._request("GET", f"/v1/namespace-cleanups/{cleanup_job_id}?{query}")).body
    ...
    async def _require_feature(self, feature: str) -> None:
        ...
        if self._capability_document is None or not self._capability_document.features.get(feature):
            raise IncompatibleRuntimeError(
                f"Runtime does not advertise required feature: {feature}",
                code="INCOMPATIBLE_RUNTIME",
            )
    ...
    async def _request(
        ...
        *,
        ...
        read_only_retry_safe: bool = False,
        ...
        retry_safe = method == "GET" or operation_id is not None or read_only_retry_safe
        retry_limit = 0 if no_retry else self._max_retries
        for attempt in range(retry_limit + 1):
            try:
                ...
                if retry_safe and retryable and attempt < retry_limit:
                ...
                if retry_safe and attempt < retry_limit:
                    await asyncio.sleep(0.1 * (2**attempt))
                    continue
...
class MilaiClient:
    ...
    def get_memory(
        ...
        **options: Any,
        ...
        return self._run(self._async.get_memory(claim_id=claim_id, state_key=state_key, **options))

    def get_working_state(self, binding: Mapping[str, Any]) -> dict[str, Any]:
        return self._run(self._async.get_working_state(binding))
    ...
    def update_working_state(
        ...
        *,
        ...
        return self._run(self._async.update_working_state(payload, operation_id=operation_id))
    ...
    def append_host_execution_event(
        ...
        *,
        ...
        return self._run(
            self._async.append_host_execution_event(payload, operation_id=operation_id)
        )
    ...
    def get_host_execution_event_window(
        ...
        return self._run(self._async.get_host_execution_event_window(binding))
    ...
    def write_note(
        ...
        return self._run(self._async.write_note(payload, operation_id=operation_id))

    def get_note(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return self._run(self._async.get_note(payload))

    def browse_notes(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return self._run(self._async.browse_notes(payload))

    def get_note_operation(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return self._run(self._async.get_note_operation(payload))

    def browse_evidence(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return self._run(self._async.browse_evidence(payload))
    ...
    def get_evidence_content(
        ...
        """Read full source content in the Host-bound private project."""
        return self._run(self._async.get_evidence_content(evidence_id, project_id=project_id))
    ...
    def list_proposals(
        ...
        *,
        ...
        return self._run(self._async.list_proposals(status, limit=limit, project_id=project_id))
    ...
    def namespace_cleanup_status(
        ...
        *,
        ...
        return self._run(
            self._async.namespace_cleanup_status(cleanup_job_id, offset=offset, limit=limit)
        )
