...
_MAX_PREFETCH_CONTEXT_CHARS = 262_144
...
def _safe_derived_result(value: object) -> dict[str, Any] | None:
    ...
    if not isinstance(value, Mapping):
        return None
    legacy_result = value.get("status") == "OK" and value.get("kind") == "DERIVED_QUERY_RESULT"
    accepted_composition = (
        value.get("status") == "COMPLETE"
        and value.get("kind") == "EVIDENCE_COMPOSITION_RESULT"
        and value.get("operand_authority") == "ACCEPTED_BINDING_ONLY"
    )
    if (
        not (legacy_result or accepted_composition)
    ...
    if accepted_composition and isinstance(value.get("display_value"), str):
        result["display_value"] = value["display_value"]
...
def prepare_prefetch(
    ...
    *,
    ...
    if not 256 <= max_context_chars <= _MAX_PREFETCH_CONTEXT_CHARS:
        raise ValueError("max_context_chars must be between 256 and 262144")
    ...
    # MemoryResolve may already have compiled a governed, provider-ready
    # context.  The MCP wire layer deliberately removes duplicate Raw Evidence
    # prose from ``items`` when that context is present, so rebuilding from the
    # compacted items would silently discard usable memory.  This path is only
    # available for the Runtime's soft-ranked read boundary.  A strict operator
    # may use it only as explicitly PARTIAL informational evidence; its typed
    # Binding/Sufficiency result remains incomplete and action-safe use stays
    # prohibited.
    soft_runtime_context: str | None = None
    raw_memory_context = recall.get("memory_context")
    if recall.get("reader_evidence_boundary") == "GOVERNANCE_ADMITTED_SOFT_RANKED" and isinstance(
        raw_memory_context, Mapping
    ):
        runtime_text = raw_memory_context.get("text")
        selected_evidence_ids = _strings(raw_memory_context.get("selected_evidence_ids"))
        selected_claim_versions = _strings(raw_memory_context.get("claim_versions"))
        if (
            isinstance(runtime_text, str)
            and runtime_text.strip()
            and (selected_evidence_ids or selected_claim_versions)
        ):
            soft_runtime_context = runtime_text
            evidence_refs.update(selected_evidence_ids)
            claim_refs.update(selected_claim_versions)
    ...
    if typed_contested or issues:
        ...
        typed_unavailable
        or canonical_unavailable
        or (degraded and not safe_items and soft_runtime_context is None)
    ):
        status = "UNAVAILABLE"
        body = {
            "status": status,
    ...
    elif soft_runtime_context is not None:
        status = "AVAILABLE"
        rendered = soft_runtime_context
    ...
    elif generic_abstention:
        status = "UNCERTAIN"
        body = {
            "status": status,
            "reason": abstention_reason or "ABSTAINED",
            "open_issue_count": 0,
            "memory_items": [],
        }
    elif safe_items:
...
def _query_terms(query: str) -> set[str]:
    return {token for token in _lexical_terms(query) if token not in _QUERY_STOPWORDS}
...
def _lexical_score(query_terms: set[str], terms: set[str], query: str) -> int:
    ...
    for query_term in unmatched_query:
        ...
        if any(
            query_aliases.intersection(_morphological_aliases(term)) for term in unmatched_terms
        ):
            score += 1
    return score
...
def _relevant_excerpt(text: str, query: str, limit: int) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        for sentence_index, sentence in enumerate(_SENTENCE_BOUNDARY.split(body)):
            ...
            choice_list_bonus = (
                30
                if assistant_intent and re.search(r",[^\n]*\b(?:or)\b", sentence, re.IGNORECASE)
                else 0
            )
            relation_bonus = (
                30
...
def _grouped_compact_context(
    ...
    *,
    ...
    base = prepare_prefetch(recall, max_context_chars=_MAX_PREFETCH_CONTEXT_CHARS)
