...
class AgentMemory:
    ...
    def after_user_observation(
        ...
        *,
        ...
        source_context: EvidenceSourceContext | Mapping[str, Any] | None = None,
    ) -> ObservationOutcome:
    ...
    def _capture(
        ...
        *,
        ...
        speaker: Literal["user", "tool"],
        source_context: EvidenceSourceContext | Mapping[str, Any] | None,
    ) -> ObservationOutcome:
        stable_observed_at = observed_at or self._operation_times.setdefault(
            operation_seed, datetime.now(UTC).isoformat()
        )
        payload = EvidenceCaptureRequest.model_validate(
            {
                "source_type": source_type,
                ...
                "speaker": speaker,
                "source_context": (
                    source_context.model_dump(mode="json", exclude_none=True)
                    if isinstance(source_context, EvidenceSourceContext)
                    else dict(source_context)
