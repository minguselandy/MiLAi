...
# The MCP server bounds structured output at 64 KiB, but a tools/call response can
# carry both structuredContent and its compatibility text representation. Keep the
# transport bounded at the same 2 MiB ceiling as the Host adapter HTTP boundary.
...
class McpUnixClient:
    ...
    def capture_evidence(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        optional = {"speaker", "source_context"}
        if (
            not required <= set(payload)
            or set(payload) - required - optional
            or payload.get("confirmation") != "CAPTURE"
        ):
    ...
    def revoke_evidence(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        """Use an operator-scoped socket for an explicitly confirmed revocation."""
        if set(payload) != {
            "evidence_id",
            "operation_id",
            "reason_code",
            ...
            raise ValueError("operator Evidence revocation payload contract failed")
        if payload.get("confirmation") != "REVOKE" or payload.get("reason_code") not in {
            "USER_REQUEST",
            "SOURCE_REMOVED",
            "PERMISSION_REVOKED",
            "RETENTION_EXPIRED",
            ...
            raise ValueError("operator Evidence revocation confirmation or reason failed")
        with self._lock:
            self._connect_locked()
            return self._call_tool_locked("milai_evidence_revoke", payload)
