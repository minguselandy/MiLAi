...
class NativeTaskMetadata:
    ...
    user_observed_at: str | None = None
    assistant_observed_at: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        settlement = (
            ...
            self.user_observed_at,
            self.assistant_observed_at,
        )
        if any(value is not None for value in settlement) and not all(
            value is not None for value in settlement
        ):
            raise TaskBindingError("settlement metadata is incomplete")
        if self.assistant_message is not None:
            if (
                not self.assistant_message.isascii()
                or _NATIVE_IDENTIFIER.fullmatch(self.assistant_message) is None
            ):
                raise TaskBindingError("assistant message is invalid")
            for observed_at in (self.user_observed_at, self.assistant_observed_at):
                assert observed_at is not None
                try:
                    parsed = datetime.fromisoformat(observed_at.replace("Z", "+00:00"))
                except ValueError as exc:
                    raise TaskBindingError("settlement timestamp is invalid") from exc
                if parsed.tzinfo is None:
                    raise TaskBindingError("settlement timestamp has no timezone")
    ...
    def bind(
        ...
        *,
        ...
        with self._lock:
            ...
            continued_task_id = self._session_task_ids.get(metadata.task_session)
            relation: NativeTaskRelation
            if continued_task_id is None:
                continued_task_id = (
                    "ow-task-"
                    + hashlib.sha256(
                        _canonical(
                ...
                self._session_task_ids[metadata.task_session] = continued_task_id
                relation = "TASK_START"
            else:
                relation = "CONTINUE"
