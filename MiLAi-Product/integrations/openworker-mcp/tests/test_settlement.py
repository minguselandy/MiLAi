...
def test_explicit_handoff_redacts_deduplicates_and_only_submits_evidence_proposal() -> None:
    ...
    assert first.outbox_id == "outbox-1"
    ...
    assert lane.evidence_calls[0]["speaker"] == "tool"
    assert lane.evidence_calls[0]["source_context"]["turn_id"] == "turn-3"
...
def test_idempotency_survives_task_epoch_change_and_uses_runtime_replay_receipt() -> None:
    lane = _Submitter()
    handoff = MemoryWriteHandoff(lane)
    first = _intent()
    restarted = replace(first, task_epoch="task-after-restart")

    initial = handoff.handoff(first, confirmation="HANDOFF")
    replay = handoff.handoff(restarted, confirmation="HANDOFF")

    assert initial.deduplicated is False
    assert replay.deduplicated is True
    assert len(lane.evidence_calls) == 1
