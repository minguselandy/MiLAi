    ...
    with listener, connection, connection.makefile("rb") as source:
        for raw in source:
            ...
            if request.get("method") == "initialize":
                ...
                elif tool_name == "milai_evidence_revoke":
                    structured = {
...
def test_catalog_and_submitter_calls_are_exact_and_share_one_session(tmp_path: Path) -> None:
    ...
    evidence_call = next(
        request
        for request in requests
        if request.get("params", {}).get("name") == "milai_evidence_capture"
    )
    assert evidence_call["params"]["arguments"]["speaker"] == "tool"
    assert evidence_call["params"]["arguments"]["source_context"]["turn_id"] == "turn-1"
...
def test_operator_revocation_call_is_exact_and_explicit(tmp_path: Path) -> None:
    ...
    client = McpUnixClient(socket_path)
    revoked = client.revoke_evidence(
        {
    ...
    assert revoked == {"evidence_id": "evidence-1", "revoked": True}
    calls = [request for request in requests if request.get("method") == "tools/call"]
    assert len(calls) == 1
    assert calls[0]["params"] == {
        "name": "milai_evidence_revoke",
        "arguments": {
            "evidence_id": "evidence-1",
            "operation_id": "revoke-op",
            "reason_code": "USER_REQUEST",
...
def test_operator_revocation_rejects_unapproved_reason_or_confirmation(
    tmp_path: Path, payload: dict[str, Any]
) -> None:
    with pytest.raises(ValueError, match="confirmation or reason failed"):
        McpUnixClient(tmp_path / "operator.sock").revoke_evidence(payload)
...
def test_default_frame_boundary_accommodates_bounded_mcp_envelope_overhead(
    tmp_path: Path,
) -> None:
    client = McpUnixClient(tmp_path / "reader-lite.sock")
    assert client.max_frame_bytes == 2 * 1024 * 1024
