...
| resolver lexical-language assumptions | OPEN | [3A-3D frozen diagnosis](revalidation/resolver-language/REVALIDATION.md), 2026-09-22: client 11 PASS/11 FAIL; Runtime interpreter 6 PASS/2 FAIL. Language/Unicode misses, changed-intent retry carry-over, arbitrary tied exact key and automatic no-memory gaps. Pure functions only: no actual retrieval/use measured, frozen aliases preserved. Owner: Product client/Runtime; separate `fix/resolver-typed-language-boundaries` after diagnostic remote closure. No resolver implementation change in this PR. |
