...
| Runtime vs Host/provider trace ownership | NEEDS_REVALIDATION | [Trace Ownership v1](reference/trace-ownership-v1.md) 已有 Lab joiner、Host 观测 testkit 与 Runtime owner export/真实 PG 单层证据；同一次执行的 MCP/Host/Provider/Lab 关联、cache provenance 和完整 receipt 尚待完成，不能以单层 tests 关闭 |
