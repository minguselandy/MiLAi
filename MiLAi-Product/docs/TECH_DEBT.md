...
| Item | State | 当前证据/下一步 |
| --- | --- | --- |
| process-local task continuity/state | OPEN | [3A-2D diagnosis](revalidation/host-continuity/REVALIDATION.md), 2026-09-22: retired native A reactivates after B and replays its old operation; registry and actual Adapter fail, reaching a third MCP/Provider fixture call. Owner: Product OpenWorker task binding; risk: duplicate work/spend and stale native graph, not demonstrated Canonical/auth bypass. Separate `fix/host-continuity-retired-instance` required; original failures retained. |
| cache-miss Host continuation | FIXED | [3A-2D scoped diagnosis](revalidation/host-continuity/REVALIDATION.md), 2026-09-22: 2 real MCP/HTTP/PG cases, 4 fresh Host processes, 10 attempts / 8 fixture calls; exact Claim reacquired after restart, receipt hit and fresh fallback work, stopped Runtime blocks Provider. Compatibility missing/mismatched slot fails closed. Native retired-instance defect remains separately OPEN. |
