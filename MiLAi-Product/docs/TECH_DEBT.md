...
| Runtime vs Host/provider trace ownership | NEEDS_REVALIDATION | [同次执行证据](reference/trace-ownership-chain.md) 已关联真实 MCP/HTTP/PG、Host 与受控 Provider fixture，覆盖 fresh retrieval、exact Evidence/Context 与失败发送状态；cache provenance、其余版本路径和完整 receipt 尚待完成，不声称真实模型使用或债务闭环 |
