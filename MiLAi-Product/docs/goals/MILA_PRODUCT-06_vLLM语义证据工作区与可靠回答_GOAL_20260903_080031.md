...
R0/vLLM probe            NOT AUTHORIZED
Product code change      NOT AUTHORIZED
new LME case lock        NOT AUTHORIZED
R2/R3 effect             NOT AUTHORIZED
formal 500               NOT AUTHORIZED
```

用户后续一次明确执行授权可覆盖 R0–R4；阶段通过自动继续，可恢复失败反思修复后继续，只有
安全硬停止或 public/schema/destructive scope 扩张才请求新授权。
