...
opened_development_effect_authorized: true
...
> v1.1 Engineering 状态：D1 persisted-frontier continuation 与 D2 lexical-only explicit
> intra-source acquisition SHADOW 已通过完整 Runtime、真实
> PostgreSQL、HTTP MCP 与部署验收，并由 migration `0053` 完成 principal/identity/replay
> hardening、migration `0054` 完成 governed fine-acquisition read boundary；P11-C1/C2 效果仍未
> 声明。用户授权的 `SUBAGENT_SEALED` 并行分支已完成 24-case 独立模型评审，但
> frozen A0 只有 `0/8/16` continuation/intra-source/control opportunity；Research X0 因
> continuation 机会不足终止，X1--X4 未进入。

## 0. v0.9 开发治理修正

用户已明确采用 development-oriented Master Plan。此前 X0 的双人人工 seal、opportunity count、
预注册 effect metric 与 Formal 隔离继续约束 **Research Mode 效果声明**，但不再阻塞默认关闭、
可回滚的最小 Engineering Mode 产品实现。

```text
Engineering Mode
  → minimal feature + unit/integration/E2E + subagent review
  → 可以声明“功能闭环可用”
  → 不得声明 P11-C1/P11-C2 量化效果

Research Mode
  → human-sealed slice + preregistered scorer + effect run
  → 才可声明 coverage/effect claim
```

本次授权的 D1 最小范围只有：

```text
first-call Evidence/Context selection 不变
post-acquisition exact identity frontier 持久化
same-query previous_context_id 只恢复 persisted frontier
online scope/permission/retention/revocation 重验
online Evidence content-identity 重验
exact seen exclusion
database operation fingerprint successor 幂等
trace: global reacquisition / pool extension / replanning / adjacent hydration = 0
```

不在 D1 范围：residual acquisition、intra-source fine acquisition、anchor-first renderer、Formal 500、
人工 gold、量化 effect PASS。原 X0 资料保留为未来 Research Mode 输入，不删除、不伪造完成。

D2 Engineering 只实现 `OFF | SHADOW` 的 lexical fine acquisition：exact coarse Evidence IDs 在
PostgreSQL 内经 scope/snapshot/revocation/retention/content-identity 重验后推导
`(source_type, session_id)`，直接形成 content-free `DIRECT_ANCHOR` diagnostic。它不进入 Context、
frontier 或 Canonical，不运行 Dense/Reader/vLLM，也不构成 P11-C2 effect PASS。完成凭据见
...
P11 agent-memory schema = query + optional previous_context_id
...
D1 implementation ceiling：D1 没有 official route-exhaustion receipt，只能用
`PERSISTED_FRONTIER_EXHAUSTED` 表示已保存 candidate pool 被消费完，用
`FRONTIER_ELIGIBILITY_CHANGED` 表示候选在线失效；D1 不返回 `FRONTIER_EXHAUSTED`。
...
adjacent hydration calls in A1 Call 2                 = 0
...
用户于 `2026-09-04T08:02:14+08:00` 明确授权执行 Product-11 Goal：

```text
Goal / Product / Lab design audit      authorized
X0 candidate and review preparation    authorized
human label final adjudication         requires real annotator + independent human reviewer
Product code changes                   authorized after X0 PASS
database migration                     authorized after X0 PASS
opened-development effect runs         authorized after applicable stage gates
real Codex runs                        authorized only when X4 eligible
formal holdout                         not authorized / must remain unconsumed
```

执行授权不豁免阶段门。必须先完成 X0、ADR-032 和 acceptance contract；X0 未达到双人人工确认、
24-case、8/8 opportunity 与 overlap gate 前，不得写 Product behavior code 或 migration。AI/model
可以生成 annotation candidate 和审阅材料，但不得把自己的判断标成 `human_adjudication_status:
COMPLETE`。
...
本节记录 v0.2 当时的 `PLANNED_NOT_AUTHORIZED` 状态；后续授权状态以 §19 和 §21 为准。

## 21. v0.3 execution authorization receipt

本修订只记录执行授权与 X0 实际进入，不改变 v0.2 的机制、指标或停止规则：

```text
authorization                     all Product-11 stages subject to preregistered gates
current block                     X0
ADR-032 / acceptance contract     authorized now
Product behavior / migration      authorized but blocked until X0 PASS
opened-development effect         authorized but blocked until corresponding implementation gates
real Codex X4                     authorized only if X4 eligibility is proven
formal 500                        forbidden / scored cases must remain 0
current X0 evidence               no Product-11 human-adjudicated 24-case seal exists yet
```

因此当前状态是 `X0_ACTIVE_HUMAN_ADJUDICATION_REQUIRED`，不是 Product implementation active，
也不是任何 effect claim 已通过。

## 22. v0.4 X0 execution audit

本修订记录真实 X0 执行结果，不改变 v0.2 合同，也不把 model-assisted proposal 冒充 human seal。

```text
final source-only A0 run          p11-x0-a0-20260904f
fresh DB before                  evidence/projection/canonical = 0/0/0
captured/projected               238/238
A0 trace cases                   24/24
Canonical mutation               0
Product label access             0
...
冻结 A0 trace 写出后，Lab 才加载 `MODEL_ASSISTED_PROPOSAL_ONLY / PENDING` candidate 做离线诊断。
该诊断不是 X0 human verdict：

```text
continuation opportunity proxy   0 / required >=8
intra-source opportunity proxy   8 / required >=8
already-complete/control proxy   16 / required >=8
human COMPLETE                   0/24
```

此前五个 X0 输入/trace 尝试均保留并写入 failure ledger；它们依次暴露 file/semantic digest 混淆、
无 candidate/无 budget frontier、超长 turn hydration degradation、frontier 过深和 activation threshold
不足。最后一次统一 threshold 修复仍无法在不针对 A0 排序器造样本的前提下得到 8 个 continuation
opportunity，因此停止继续塑形。

当前不得进入 Product behavior、migration、X1/X2 treatment 或 X3/X4。正式
`PARKED_PRODUCT11_INSUFFICIENT_HUMAN_SEALED_OPPORTUNITY` 仍需要真实 annotator 与独立 reviewer
确认 group；在此之前状态固定为 `X0_BLOCKED_HUMAN_AND_CONTINUATION_OPPORTUNITY`。权威细节见
`MILA_PRODUCT-11_X0_EXECUTION_AUDIT_20260904_085558.md`。

## 23. v0.5 independent-human workflow readiness

Lab 已完成 X0 外部人工门的可执行封装，但没有伪造任何人工结论：

```text
run                              p11-x0-human-workflow-20260904a
annotator submission template    proposal-free / PENDING_HUMAN
reviewer submission template     proposal-free / PENDING_HUMAN
review material                  source-only; no Product A0 output
merge                            exact semantic agreement or conflict-only output
opportunity assignment           derived after agreement from frozen A0 trace
human COMPLETE                   0/24
Formal files/cases               false/0
```

两名真实人类必须分别完成 24/24、使用不同 HUMAN identity 并独立签署。合并器忽略任意 human-local
group ID，只比较 exact Evidence/turn membership、required semantics 与由 turn ref 确定的 provenance。
有分歧时不得产出 adjudicated gold；完全一致后才把 frozen A0 trace 与 human groups 联结，自动生成
opportunity 字段。人类不能手工声明 continuation/intra-source/control 机会。

这一步只减少外部协调歧义，不改变 X0 阶段门。Product behavior、migration、X1--X4 继续保持
未进入。权威操作说明为 Lab `docs/PRODUCT11_X0_HUMAN_REVIEW_RUNBOOK.md`。

## 24. v0.6 external-human execution block

连续三次 Goal 执行审计均确认：没有两份由不同真实人类完成的 24-case submission。A0、双人材料、
单人 validator、conflict-safe merger、seal 与质量门均已就绪，不存在可继续自动化且不违反 X0 的
工程步骤。因此持久执行 Goal 进入：

```text
BLOCKED_EXTERNAL_HUMAN_ADJUDICATION
```

这不是 Product-11 的实验 PASS/FAIL/PARKED 终态，也不允许把 proxy 结果升级为 human verdict。
恢复条件仅为外部提供两份真实、独立、完整且签署的 submission；恢复后从 validator → merger →
X0 seal 继续，不重跑或重塑 A0。Product behavior、migration、X1--X4 与最终 MCP continuation 封装
继续未进入。Formal access/scoring 保持 `false/0`。

## 25. v0.7 baseline HTTP MCP packaging

根据用户要求，先完成并验证现有 HTTP MCP 产品封装。`milai-agent-memory-mcp@0.1.1` 已通过 66 项
package tests、真实 child-process Streamable HTTP/Bearer smoke、Ruff、strict mypy、wheel/sdist 与
clean-wheel install/import gate。Codex 0.153.0 的 `mcp add --url --bearer-token-env-var` 本地接口已
核对，唯一权威使用说明写入 `docs/runbooks/http-mcp.md`。

该交付只封装已有 baseline：固定 `agent-memory`、HTTP、0 retries，工具仍严格为
`milai_memory_resolve(query, previous_context_id?)`。它不实现或宣称 Product-11 persisted-frontier
continuation，因而不绕过 §24 的 X0 外部人工阻塞。详情见
`MILA_PRODUCT-11_HTTP_MCP_PACKAGING_AUDIT_20260904_093251.md`。

## 26. v0.8 independent HTTP-FULL-01 integration capability

用户随后明确授权把本地 Codex 产品入口扩展为单端口完整 lifecycle。该工作按 ADR-033 和独立
`HTTP-FULL-01` acceptance contract 实现，不进入 X1--X4 effect claim，也不改 P11 的
`agent-memory` 实验 schema：

```text
endpoint                         127.0.0.1:7337/mcp
profile                          codex-full
inbound Host                     one Codex principal / one Bearer
bound scope                      exactly one project
catalog                          11 lifecycle tools
governance mode                  SINGLE_HOST_FULL_CONTROL
independent Host review          false
Runtime credentials              reader/submitter/reviewer/operator role-routed
canonical actor separation       retained
Runtime migration                none
P11 continuation implementation not started / still blocked by X0
```

`codex-full` 允许同一 Codex Host 驱动 Capture→Proposal→Review→Revoke/Cleanup，但没有删除 0032 的
canonical self-review guard，也不伪装成人类或独立 Host review。MCP mutation receipt 与结构化日志
记录同一 inbound principal、scope digest、governance mode 和 `independent_host_review=false`。

这一用户授权独立覆盖了 §2 的产品 MCP catalog 冻结，但只覆盖新 `codex-full` integration profile；
P11 C1/C2 的 `agent-memory` schema、Host prompt、Reader/vLLM、A0 与所有 X0 门仍不变。完整使用说明
为 `docs/runbooks/http-mcp.md`。

## 27. v1.2 user-authorized subagent-sealed Research branch

用户于 `2026-09-05` 明确要求继续实验且用 subagent 取代人工审查。本修订仅为
opened-development Research X0 增加并行 provenance 分支；它不重写 §23--§24 的历史
HUMAN 路径，也不宣称 human gold。

```text
seal mode                         SUBAGENT_SEALED
claim ceiling                     OPENED_DEVELOPMENT_MODEL_ADJUDICATED
subagent annotator/reviewer       p11_x0_annotator_c / p11_x0_reviewer_c
source packet isolation           dedicated source-only / proposal-free
cases reviewed                    24 / 24 each
normalized group agreement        60 / 60; exact 24-case agreement
acceptable exact turn refs        70
human adjudication                NOT_PERFORMED
Formal files/cases                false / 0
```

执行前 immutable orchestration manifest 固定 role、orchestrator-assigned identity、invocation、
专用 packet digest 与预期输出路径；独立 validator 再验证 packet 的 source-only schema、fixture
hash、case order 和 forbidden fields。merger 忽略
subagent-local `group_id`，只比较 exact turn/Evidence membership、required semantics 和由 turn ref
派生的 source/session。任一 case 分歧就 fail closed，只输出双方 semantic digest，不自动
裁决。source packet 不含 Evidence-ID 映射，因此 proposal 的 `acceptable_evidence_ids` 必须为空。
现实执行得到全量一致，然后才与 label-blind frozen A0 trace 离线 join；seal 重新加载并重算两份
proposal，不能由手写 adjudicated JSONL 或篡改 merge summary 绕过。
...
sealed label SHA-256              8f3a790db50b9cb05a184066fa87e160a6fa4ca718ec65937a3d3675fbf583e7
...
terminal                          PARKED_PRODUCT11_INSUFFICIENT_SUBAGENT_SEALED_OPPORTUNITY
```

这与早期 PENDING proxy 的 `0/8/16` 结果一致，但现在已由两个显式非 HUMAN 的
subagent 独立完成 group semantics 并封存为模型评审终态。按 §8.4 停止规则，不得为
补足 continuation opportunity 重塑 slice、调 A0、运行 X1/X2 treatment 或消费 Formal
500。D1 persisted-frontier 和 D2 SHADOW 的 Engineering PASS 保留，但不升级为 P11-C1/C2
量化 effect claim。

`20260905a` receipt 作为历史记录保留；架构复审发现它复用了 HUMAN-oriented packet，且 seal
未强制验证 merge summary。`20260905c` v0.2 receipt 以 append-only supersession 修正证据链，
没有删除或改写旧产物。`20260905b` 只生成了未使用的预执行 manifest，没有 proposal 或 claim。
