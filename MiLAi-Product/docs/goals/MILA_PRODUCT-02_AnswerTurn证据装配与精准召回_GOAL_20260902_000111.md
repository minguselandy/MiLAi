...
3. 固定上下游，形成一个可证伪根因
4. 只改一个 component / policy
5. 跑失败正例 + 正确对照 + 安全负控
6. 通过后跑 8-case micro-slice
7. 再跑受影响的 24/128 gate
8. 保留修复，或干净回滚该 treatment
9. 继续下一首损家族
```
...
| 首损 | 优先反思 | 允许的通用改进 | 禁止的假修复 |
| --- | --- | --- | --- |
| DISCOVERY | query 表达或通道可达性 | 独立 small quota、Raw/Dense、适用 temporal lane | 无边界扩大 Top-k、case synonym |
| LOCAL EXPANSION | session/turn identity、窗口方向 | paired turn、same-session bounded expansion | 整 session 前缀、跨 session 拼接 |
| ADMISSION | 全局 cutoff、长 unit 占满预算 | role breadth-first、skip/降级 oversized unit | 截断 dialogue pair、只加 token |
| BINDING | subject/relation/source/time 不匹配 | grounded span、typed operand、ambiguity | score 高就接受、模型 confidence |
| OPERATOR | operand 或 proof 未闭合 | AcceptedBinding-only、deterministic proof | raw dated result、Reader 自行计算 |
| READER | 噪声、顺序、生成不稳定 | 最小 EvidenceSet、deterministic result | 重复调用投票、将答案写入 prompt |
| HARNESS | metric、identity、并发错误 | 修评测实现并 fresh rerun | 把 system failure 算 abstention |
...
- [ ] 当前三个 dirty files 已审阅，用户改动没有被覆盖；
- [ ] Product-01 S4 Wrong COMPLETE repair 具有 targeted 正反例和 fresh superseding test；
- [ ] session coverage 与 answer-turn/span coverage 指标已分离；
- [ ] direct Raw turn、Canonical exact 和健康 Dense 使用独立小 quota；
- [ ] same-session local expansion 不跨 source session；
- [ ] EvidenceSet 按 requirement/operand 覆盖；
- [ ] ordinary LookupReadiness 与 StrictSufficiency 分离；
- [ ] strict operator operand 全部来自 AcceptedBinding；
- [ ] Context 按 role 先广后深，只装入 atomic units；
- [ ] known false Wrong COMPLETE、scope/revoke/wrong-relation leak 为 0；
- [ ] Product 本地 golden flow、24 scenarios、100 warm 通过；
- [ ] fresh 128-case matched decision 完成；
- [ ] 128 entry 通过时才执行一次 500-case Qwen confirmation；
- [ ] `LEAN` profile 具有单一关闭/回退路径；
- [ ] FORMATION、Graph、Reflect 和 multi-round Agent 未进入默认产品路径；
- [ ] Product 不导入 Lab，Lab 不绕过 Product official path；
- [ ] Schema 继续为 `NO-GO FOR FREEZE`，不虚报 production-ready。
