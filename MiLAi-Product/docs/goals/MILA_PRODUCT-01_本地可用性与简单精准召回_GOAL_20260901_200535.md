...
execution_activation: EXPLICIT_USER_REQUEST_REQUIRED
...
本文件生成后只处于 `PLANNED`。用户明确要求执行后，才可将状态改为
`ACTIVE_IMPLEMENTATION` 并开始 S0。
...
- [ ] Product 已有独立 baseline commit，Lab pin 绑定 commit + manifest；
- [ ] S0 fresh PostgreSQL、真实角色、operations、MCP/OpenWorker 和 lease 门通过；
- [ ] S1 structured identity、same-session adjacency、三层 trace 和 exact packing 通过；
- [ ] repaired untreated baseline 使用新 Context 重建，旧 R4 未 resume；
- [ ] S2 ordinary relation Binding、strict COMPLETE owner 和 EvidenceSet selection 通过；
- [ ] 已按效果选择 repaired baseline 或 simple candidate，未强留无增益组件；
- [ ] S3 local golden flow、24 scenarios、100 warm、restart/revoke/outage 通过；
- [ ] 一次 full Runtime、六 adapters、architecture lock 和 clean builds 通过；
- [ ] Product manifest 与 Lab pin 更新；
