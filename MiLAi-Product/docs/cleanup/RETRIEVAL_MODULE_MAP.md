...
limits, temporal ordering, Context budget and selection weights were moved without modification.
AST comparisons against each pre-extraction `retrieval.py` baseline confirmed all 34 extracted
...
Later Retrieval-only PRs may extract acquisition, operators, assembly and trace helpers. Each
extraction remains independently gated; this map does not authorize behavior changes or a
wholesale `RetrievalService` rewrite.
