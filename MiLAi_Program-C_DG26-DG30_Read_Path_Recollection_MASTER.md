...
> 文档版本：1.3 / ACTIVE PROGRAM-C SUBPLAN
...
> 当前状态：DG-28 deterministic union + MF-03 formed consumption + DG-30 Read Path 已封存；DG-29 无机会不进入
...
| DG-27 V02/V03 | TERMINAL NEGATIVE | 协议错误已修复；有效 effect 仍产生 Wrong COMPLETE/回归 | 不采用模型解释器作共同决策边界 |
| DG-28 Lite | PASS | target candidates 4/7 → 7/7，bindings 3/7 → 6/7，precision 1.0，Wrong COMPLETE 0 | 保留最小 deterministic union |
| DG-28 Formed | PASS | 同一 44 candidates 上 bindings 6/7 → 7/7 | 消费 MF-03 sidecar，无 query-time model |
| DG-29 | NOT ENTERED | Formed+Simple 已无 unresolved target requirement | `NOT_ENTERED_NO_REFINDING_OPPORTUNITY` |
| DG-30 | PASS_READ_PATH_INTEGRATION | 7/7 candidates/bindings，precision 1.0，Wrong COMPLETE 0 | `release_ready=false`，flag OFF，holdout 未用 |
...
~~~text
DG-27 negative boundary result (retained)
  → DG-28 deterministic Raw+Simple       6/7
  → MF-03 sidecar on same candidate pool 7/7
  → DG-29 NOT_ENTERED
  → DG-30 PASS_READ_PATH_INTEGRATION
~~~

后续不再为这 7 个开发义务增加 channel、planner 或 refinding 轮次。新 Read Path 工作只能由扩展后的 matched validation 中出现的新 first loss 触发。
