...
def _compact_payload(payload: Mapping[str, object]) -> str:
    ...
    states = [item for item in raw_states if isinstance(item, Mapping)] if isinstance(
        raw_states, list
    ) else []
    ...
    for state in states:
        ...
        epistemic = str(
            state.get("epistemic_status") or state.get("effective_status") or "UNKNOWN"
        )
    ...
    issues = [item for item in raw_issues if isinstance(item, Mapping)] if isinstance(
        raw_issues, list
    ) else []
    for issue in issues:
        header = " ".join(
