...
tuning synonyms, Top-k, Dense, ranking weights or Reader prompt
...
> 只有在 Candidate F 实际应用且形成非零 Evidence/Binding 分母后，才能判定它对 public LongMemEval 的 mediator 和答案影响。
...
## R4 — One Post-Repair LongMemEval Characterization
...
MLR01-F  Candidate F + Raw fallback
```

两臂固定：

```text
same source/data/question order
same Reader/model/token/candidate ceilings
same timeout and concurrency
...
R4-02 run all 1000 context cells first
R4-03 verify delivery/capacity/binding gates before model calls
R4-04 run answer generation only if R4-03 passes
R4-05 seal answers
R4-06 run judge/scorer in a separate service window
R4-07 aggregate paired effects, first loss and cost
R4-08 emit engineering and candidate dispositions separately
```

若 R4-03 未过，不消耗 answer/judge 调用；返回 R1–R3 的对应 repair loop，不直接为方法生成负效果结论。
...
```text
normalized F1 / EM / official local accuracy
paired Candidate-minus-Raw delta and CI
ability-level delta
...
Wrong COMPLETE = 0
CorrectCaseRegression = 0
paired normalized-F1 observed delta > 0
paired normalized-F1 CI lower >= -0.02
no ability-family delta < -0.05
```

这是候选支持，不是 formal generalization 或 default-ON 授权。
...
| M5 | R4 contexts | M4 PASS | all 1000 context cells valid | parallel stateful lane |
| M6 | R4 answer/judge | context gate PASS | paired public characterization | 8-way separated model windows |
