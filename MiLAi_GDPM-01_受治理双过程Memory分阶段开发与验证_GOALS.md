...
version: "0.3"
status: HANDED_OFF_TO_MILA-MVP-01_AFTER_B0_CANARY_REPAIR_REQUIRED
document_type: DEVELOPMENT_AND_VALIDATION_GOAL
created_at: "2026-08-31T18:47:00+08:00"
updated_at: "2026-09-01T11:11:55+08:00"
...
authority_snapshot: MILA-ML-MASTER@2.0
execution_authorized: false
execution_scope: NONE_HANDOFF_COMPLETE
active_block: NONE
active_case_scope: NONE
...
activated_at: "2026-08-31T22:21:59+08:00"
canary_authorized_at: "2026-09-01T08:23:16+08:00"
standing_user_execution_authority: GDPM_ALL_GOAL_BLOCKS_SEQUENTIAL_NO_RECONFIRMATION
continuous_execution_enabled: false
continuous_execution_scope: B0_THROUGH_B4_SEQUENTIAL_WITHIN_STANDING_USER_AUTHORITY
continuous_execution_policy: PASS_OR_RESOLVED_BLOCK_AUTO_ADVANCE
transition_user_reconfirmation_required: false
transition_authority: standing_user_execution_authority
transition_record_protocol: ATOMIC_MASTER_GOAL_RUN_LOCK_REFRESH_BEFORE_NEXT_CELL
repair_policy: REFLECT_REPAIR_RETEST_RESUME
repair_auto_continue: true
hard_hold_policy: INVALIDATE_ATTEMPT_REPAIR_AND_START_FRESH_ATTEMPT
latest_targeted_fixture_terminal: var/gdpm/gdpm-b0-20260831-002/terminal.json
latest_targeted_fixture_terminal_sha256: 6fa6cc1121f2c5abaedfb7acf420800ba9d69fb738d04f75aed5c827c1487126
latest_targeted_fixture_status: PASS_B0_TARGETED_FIXTURES_AWAITING_24_AUTHORIZATION
latest_canary_terminal: var/gdpm/gdpm-b0-20260901-003/terminal.json
latest_canary_terminal_sha256: bcb6382d3a2b29e6b5712a2a2bbaa4d631a764d5d442e67f70a2e29449b41d78
latest_canary_status: FAIL_B0_24_CONTEXT_ONLY_CANARY_REPAIR_REQUIRED
handoff_successor: MILA-MVP-01@0.1
handoff_recorded_at: "2026-09-01T11:11:55+08:00"
next_authority_required: NONE_GDPM_WRITE_AUTHORITY_CLOSED
...
supersession_recorded_by_master:
...
_本文件保留受治理双过程 Memory 的历史开发合同。B0 24-case context-only attempt-003 已封存为 repair-required；MILA-ML-MASTER@2.1 已结束本 Goal 的活动写权并将最小可用性修复移交 MILA-MVP-01。未执行的复杂分支保持 default OFF。_
...
当前唯一执行顺序来源仍是 [MILA-ML-MASTER@2.1](./MiLAi_Memory_Lifecycle_总_GOALS.md)。Master 已登记本 Goal 的 B0 24-case repair-required 终态并激活后继 MVP-01；GDPM 不再拥有新的 cell 写权。

连续授权不等于并行乱序或范围扩张：

1. 同一时刻只允许一个 active Block；
2. Block 达到 `PASS`、`RESOLVED_NO_GAIN` 或 `NOT_NEEDED` 后，总控必须原子封存当前结果、刷新 Master/Goal/run-lock 的下一 exact scope，然后自动开始下一 Block；
3. Block 出现可修复失败时留在当前 Block，反思根因、完成通用修复、targeted 正反例和 fresh attempt，然后自动续跑；
4. 24、128、500 的 exact case/run scope 仍须在首个 cell 前写入 Master 与 run-lock，这是执行记录而不是新的用户确认；
5. formal holdout、产品 default-on、Schema/公开 MCP 变更、破坏性数据恢复、超出 B0→B4 的任务扩张，仍需新的显式授权。

本 Goal 只规范自己的实现、实验和 terminal。稳定 Program/Plane 边界仍由 MILA-ML-ARCH 定义；对象身份、单写者、权限、删除和事务仍受 architecture/v1.0 与 Lean 实施合同约束。
...
当前 standing authority 覆盖 B0→B4 的顺序执行，总控在前块解除后自动写入 500-case exact scope；若该 authority 后续被收紧或 exact scope 尚未写入 Master，则只能报告 24/128-case development evidence，不能宣称 H1 通过。
...
flowchart TB
    ...
    accDescr: Evaluation truth is repaired first. Each resolved block advances automatically; a failed block repairs in place and resumes before the fast memory path, raw-preserving lifecycle, optional associative path, and final validation continue.

    standing_scope{Standing scope active?}
    ...
    standing_scope -->|Current B0| b0
    b0 -->|PASS auto| b1
    b1 -->|PASS auto| b2
    b2 --> opportunity
    opportunity -->|Yes, execute| b3
    opportunity -->|No, NOT NEEDED| no_slow
    b3 -->|RESOLVED auto| b4
    no_slow -->|Auto| b4
    ...
    class standing_scope,opportunity gate_style
...
3. 第 2 级解除后，总控在 standing authority 内自动写入 exact scope，重建 128-case untreated baseline 与 first-loss map。

协议或实现失败进入 repair iteration，不终止 GDPM。B0 未通过时，不进入 Answer/Judge 或 treatment。

当前执行状态（2026-09-01）：第 1 级已由
`var/gdpm/gdpm-b0-20260831-002/terminal.json` 封存为
`PASS_B0_TARGETED_FIXTURES_AWAITING_24_AUTHORIZATION`。定向范围共
`71 passed`；上述六项最小门均满足，附加的 atomic truncation、long-turn split 与
rank-first prefix violation 也均为 `0`。本 attempt 执行 benchmark case、
Reader、Answer、Judge 均为 `0`，formal holdout 未消费。`block_complete=false`、
`goal_complete=false`。用户已于 2026-09-01 授予整个 GDPM Goal 的持续分块执行权；第 2 级
`B0_24_CASE_CONTEXT_ONLY_CANARY` 随后由 `gdpm-b0-20260901-003` 执行并封存：4/24 PASS，
20/24 为 `PROTOCOL_IMPLEMENTATION`；其中 14 个缺 `ContextReceipt`、6 个超过 MCP wire output
limit。Reader/Answer/Judge 为 `0`，formal holdout 未消费。该结果未打开 128-case，而是按 Master
决策移交 MVP-01 U0 做最小产品路径修复。
...
本 Goal 使用一个连续执行控制器，而不是“一次 gate 失败就终止”：

| 当前 Block 结果 | 总控动作 | 后续 |
| --- | --- | --- |
| `PASS` | 封存结果，原子刷新 Master/Goal/run-lock | 自动执行下一 Block |
| `RESOLVED_NO_GAIN` | 保留有效负结果，关闭无增益的可选 treatment | 自动沿更简单的安全路径继续 |
| `NOT_NEEDED` | 封存机会不存在的证据 | 自动跳过该可选分支 |
| `REPAIR_REQUIRED` | 停留当前 Block，反思根因、做最小通用修复、跑正反例和 fresh attempt | 修复通过后自动续跑当前 Block |
| `HARD_INVARIANT_BREACH` | 当前 attempt 失效，禁止评分与 checkpoint 复用；隔离、修复并重放安全负控 | 以 fresh attempt 恢复当前 Block |
| `PROGRAM_DONE` | 封存 Program terminal 和产品建议 | 结束已授权范围 |

~~~text
execute current block
→ pass/resolved/not-needed: atomically advance
→ repair-required: diagnose → repair → targeted tests → fresh attempt → resume
→ hard breach: invalidate attempt → isolate → repair → safety replay → fresh attempt
→ repeat until B4 is resolved or new external authority is genuinely required
~~~

`PASS`、`RESOLVED_NO_GAIN` 和 `NOT_NEEDED` 都是可继续状态；它们不得被误写为整个 Goal 终止。强制暂停只用于超出 standing scope、formal holdout/default-on/Schema 变更、需要破坏性恢复，或确实缺少外部能力/用户选择的情况。
...
| Evidence 损坏、权限/撤销泄漏、跨 scope 泄漏或未授权 Canonical mutation | 冻结整个活动 run，隔离影响范围，做根因修复与 fresh 安全重放；只在需要破坏性数据恢复或新权限时请用户确认 | 冻结当前 run，不默认终止 Goal |
...
- 同一通用根因连续三次有实质 repair 仍无改善时，自动执行简化/rebase review；可选 branch 记为 `RESOLVED_NO_GAIN` 或 `PARKED_BRANCH` 并继续主路径，B0/B1/B2 必需能力则必须换用满足合同的替代实现后继续，不能简单跳过；
- sealed effect miss 后不修改同一数据重跑；修复只能在 repair-dev 完成，再用新 sealed set；
- 每个实质根因只写一条 repair-log，后续验证更新同一条 disposition。

每次 Block 转移必须先原子写入当前 terminal、下一 active block、exact case/run scope 与新 run-lock，再启动下一个 cell。这个记录动作由总控完成，不触发新的用户确认。
...
- Block 达到 `PASS`、`RESOLVED_NO_GAIN` 或 `NOT_NEEDED` 时，一次原子同步当前 terminal、Goal 的 active block、MILA-ML-MASTER 的 exact scope 和下一 run-lock；
- AGENTS 仅在整个 GDPM terminal、正式接管或用户可见导航发生实质变化时更新，不在每次 repair 后重写；
...
以下每项一旦完成，或以受允许的 `RESOLVED_NO_GAIN` / `NOT_NEEDED` 解除，总控就必须按本文的连续执行合同自动转入下一项，不把局部负结果当成 Program 终止：
...
~~~text
Block PASS / RESOLVED_NO_GAIN / NOT_NEEDED
→ seal current terminal
→ refresh exact next scope and run-lock
→ start next Block automatically

Block REPAIR_REQUIRED / HARD_INVARIANT_BREACH
→ remain on current Block
→ reflect, repair, test and create a fresh attempt
→ resume automatically after the repair gate passes
~~~
...
> 先修复观测真实性，再做简单而正确的 Memory；只有简单路径留下可证明缺口，才启用 Formation 或联想慢路径。通过就原子封存并自动向后执行；未通过就留在当前 Block 定位 owner、反思、做最小通用修复、fresh 重测后继续。有效证明无增益的可选方法不反复调参，而是保留负结果、关闭分支并沿更简单的安全路径继续。
