...
import run_v0223_current_admission as worker
...
import v0223_current_cpu_batch as binding
...
def contract():
    return {
        "status": "CURRENT_CANDIDATE_CALIBRATION_FROZEN",
        ...
        "calibration_order": [
            {
                ...
                "mode": mode,
                "variant": "candidate",
                "root": str(worker.CPU_BASE / ("currentv1-" + instance)),
                "binding_sha256": "b" * 64,
            }
...
def test_contract_rejects_unreviewed_widened_or_mixed_arms(key, value):
    ...
    with pytest.raises(ValueError):
        worker.validate_contract(frozen, "k3-u1", "U")
...
def test_exact_all_six_positions_and_selected_mode():
    ...
    for value in (None, [], "contract"):
        with pytest.raises(ValueError):
            worker.validate_contract(value, "k3-u1", "U")
    for instance, mode in (("k3-u1", "S"), ("k1-u1", "U"), ("k3-prefix", "S")):
        with pytest.raises(ValueError):
...
def test_binding_changes_only_roots_not_constructor_authorization(monkeypatch):
    ...
    assert methods("v0223_current_cpu_batch.py") == methods("v0223_k2_cpu_batch.py")
    roots = []
    ...
    for instance in ("k2-base", "k1-u1", "k3-cold", "../k3-u1", ""):
        monkeypatch.setenv("MILA_V0223_INSTANCE", instance)
...
if sys.argv[2] == 'worker':
    import run_v0223_current_admission as target
    sys.argv = ['worker', '--contract', '/absent', '--contract-sha256', '0'*64,
                '--instance', 'k3-u1', '--mode', 'U']
elif sys.argv[2] == 'prepare':
    import prepare_v0223_current_fixture as target
    sys.argv = ['prepare', '--instance', 'k3-u1']
else:
    import inventory_v0223_current_fixture as target
    sys.argv = ['inventory', '--instance', 'k3-u1', '--output', sys.argv[3]]
try:
    target.main()
...
assert 'v0223_current_cpu_batch' not in sys.modules
...
def test_exact_installer_order_private_json_span_and_restore(tmp_path, monkeypatch, mode):
    ...
    with worker.installed_current_stack(observer, batch_class=Batch), observer.span("execution"):
        assert transaction_module.Batch.transaction is corrected_transaction
        assert (
            evidence.verify_tree.__wrapped__ if mode == "S" else evidence.verify_tree
        ) is candidate_tree
    ...
    if mode == "S":
        ...
        assert sum(row["json_docs"] for row in report["records"]) == 0
...
def test_current_five_scope_original_session_u_s_coverage_and_u_clock_isolation(
    ...
    for mode in ("U", "S"):
        ...
        with (
            worker.installed_current_stack(observer, batch_class=Batch),
...
@pytest.mark.parametrize(
    ...
    [("prepare_v0223_current_fixture", "prepare"), ("inventory_v0223_current_fixture", "main")],
)
