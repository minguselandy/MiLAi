...
import prepare_v0224_cpu_workflow as old_fixture
import prepare_v0224_k3_cpu as controller
import run_v0224_verified_digest_preparation as old_controller
import v0224_k3_cpu_batch as batch
...
def test_one_k3_fixture_uses_original_builder_finalizer_and_shared_deadline(parent):
    path, pin, calls = parent
    report = controller.run_preparation(path, pin)
    assert len(calls) == 3 and len(report["children"]) == 3
    ...
    assert all("prepare_v0224_k3_cpu.py" in row["argv"][1] for row in calls[2:])
...
def test_middle_fixture_failure_stops_wave_without_replacement(parent, monkeypatch):
    ...
    def fail(argv, phase, *args):
        if phase == "fixture_preparer-" + ORDER[0]:
            raise primary
    ...
    assert result["phase"] == "fixture_preparer-" + ORDER[0]
...
def test_delegated_helpers_are_original_objects():
    ...
    assert controller.LIMIT_SECONDS == 3600
...
def test_all_old_entries_and_fixed_evidence_preserved():
    ...
    assert len(controller.R04_EVIDENCE_PINS) == 6
    assert (
...
def test_fixture_command_routed_explicitly(parent):
    ...
    assert [row["argv"][4] for row in calls[2:]] == [batch.K3_INSTANCE]
