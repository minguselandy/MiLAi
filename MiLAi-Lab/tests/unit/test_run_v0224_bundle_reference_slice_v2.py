...
import run_v0224_bundle_reference_slice_v2 as worker
...
def synthetic(tmp_path, monkeypatch):
    ...
    import v0224_bundle_cpu_batch_v2 as batch_module
    ...
    def make(writes=1):
        ...
        class SyntheticBatch:
            def __init__(self, selected, pin, *, static_authority):
                assert selected == root and pin == state.binding_sha
                self.root, self.plan, self.binding = root, plan, state.binding
...
def test_only_fixed_instance_root_and_batch_import_change():
    old = Path(worker.__file__).with_name("run_v0224_bundle_reference_slice.py").read_text()
    expected = old.replace('INSTANCE = "bundle-v1-slice"', 'INSTANCE = "bundle-v2-slice"')
    expected = expected.replace("20260913-gate-a-bundle-v1", "20260913-gate-a-bundle-v2")
    expected = expected.replace(
        "from v0224_bundle_cpu_batch import", "from v0224_bundle_cpu_batch_v2 import"
    )
    assert Path(worker.__file__).read_text() == expected
    assert worker.INSTANCE == "bundle-v2-slice"
    assert worker.CPU_ROOT == Path(
        "/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-bundle-v2/bundle-v2-slice"
    )


@pytest.fixture
def real_public_pins():
    """Read fixed public metadata only; no World or admission/Session execution."""
    import prepare_v0221_http_v2 as cases_module
    ...
    with AdmissionReadScope() as scope:
        ...
        pins = fixture._public_source_pins(original, mapping, cases_module.CASES)
        assert len(pins) == 4
        for path, digest in pins.items():
            assert worker._sha(path) == digest
            scope.read_bytes(Path(path), digest)
    assert scope.status == "CLOSED_VERIFIED_TWO_OBSERVATIONS"
    return original, pins, cases_module.CASES


def test_real_four_original_public_pins_collected_without_remint(real_public_pins):
    import prepare_v0224_bundle_fixture_v2 as fixture

    _plan, pins, _cases = real_public_pins

    def forbid_remint(path):
        raise AssertionError("ORIGINAL_PUBLIC_EXPECTED_HASH_MUST_NOT_BE_REMINTED")

    collected = fixture._collect_input_pins(set(map(Path, pins)), pins, forbid_remint)
    assert collected == pins
    fixture._check_sealed_pins({"inputs": collected, "dependencies": {}}, pins, {})
    missing = dict(collected)
    missing.pop(next(iter(missing)))
    with pytest.raises(ValueError, match="MEASUREMENT_INPUT_CHANGED_AFTER_SCOPE_CLOSE"):
        fixture._check_sealed_pins({"inputs": missing, "dependencies": {}}, pins, {})


@pytest.mark.parametrize("include_direct_pins", [True, False])
def test_real_public_metadata_through_fixture_inputs_to_worker_before_session(
    real_public_pins, synthetic, monkeypatch, include_direct_pins
):
    """Real expected pins and bytes, synthetic coordinator, sentinel before any Session."""
    import prepare_v0221_http_v2 as cases_module
    import prepare_v0224_bundle_fixture_v2 as fixture
    from prepare_v0222_full import resolve_p4_spec

    old_plan, pins, cases = real_public_pins
    state = synthetic()
    old_spec = copy.deepcopy(old_plan["P4"][0])
    old_spec["id"] = worker.INSTANCE + "-p4-01"
    old_spec["scope"] = "TEST_ONLY_source_binding_no_session"
    source = cases / old_spec["root"] / "public-initial.json"
    spec, _ = resolve_p4_spec(old_spec, worker._read(source))
    state.plan["P4"][0] = spec
    state.contract["first_p4_spec"] = spec
    state.contract["public_source"] = {"path": str(source), "sha256": pins[str(source)]}
    state.contract["files"].update(pins)
    collected = fixture._collect_input_pins(set(map(Path, pins)), pins, worker._sha)
    worker._save(state.root / "manifest.json", {"inputs": collected if include_direct_pins else {}})
    state.binding["manifest.json"] = worker._sha(state.root / "manifest.json")
    worker._save(state.root / "execution-binding.json", state.binding)
    state.binding_sha = worker._sha(state.root / "execution-binding.json")
    state.contract["binding_sha256"] = state.binding_sha
    state.contract["files"][str(state.root / "execution-binding.json")] = state.binding_sha
    with sqlite3.connect(state.root / "batch.sqlite") as db:
        db.execute("UPDATE meta SET value=? WHERE key='binding'", (state.binding_sha,))
    baseline = state.contract["coordinator_baseline"]
    worker._save(baseline["path"], worker.sql_snapshot(state.root))
    baseline["sha256"] = worker._sha(baseline["path"])
    state.contract["files"][baseline["path"]] = baseline["sha256"]
    monkeypatch.setattr(cases_module, "CASES", cases)
    calls = []

    class StopBeforeRealSession(RuntimeError):
        pass

    def no_real_reference(*args, **kwargs):
        calls.append("SOURCE_PIN_INTERFACE_REACHED")
        raise StopBeforeRealSession("No World, provider or Session may run in this test")

    monkeypatch.setattr(bounded, "prepare_p4_bounded", no_real_reference)
    worker.validate_contract(state.contract)
    if include_direct_pins:
        with pytest.raises(StopBeforeRealSession):
            worker._run_slice(state.contract, state.out, [], {})
        assert calls == ["SOURCE_PIN_INTERFACE_REACHED"]
    else:
        with pytest.raises(ValueError, match="CURRENT_MANIFEST_PUBLIC_PIN_DRIFT"):
            worker._run_slice(state.contract, state.out, [], {})
        assert calls == []
    assert state.operations == []
    assert not (state.out / "full-reference").exists()
