...
import prepare_v0224_runtime_union_fixture as old_fixture
import prepare_v0224_verified_digest_fixture as fixture
import run_v0224_runtime_union_preparation as old_controller
import run_v0224_verified_digest_preparation as controller
import v0224_runtime_union_batch as old_batch
import v0224_verified_digest_batch as batch
...
def test_only_six_fixed_fresh_roots(name, monkeypatch):
    ...
    assert str(batch.selected_root()) == (
        "/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-scope-digest-v1/" + name
    )
...
def test_all_six_fixture_children_share_one_authority_and_deadline(parent):
    ...
    assert all("prepare_v0224_verified_digest_fixture.py" in row["argv"][1] for row in calls[2:])
...
def test_lineage_business_body_identical_and_old_scope_refused():
    import v0224_bundle_lineage as old_lineage
    import v0224_bundle_read_scope as old_scope
    import v0224_verified_digest_lineage as lineage

    assert tree(lineage.verify_lineage_r03) == tree(old_lineage.verify_lineage_r03)
    foreign = object.__new__(old_scope.BundleReadScope)
    with pytest.raises(TypeError, match="EXPLICIT_R03_BUNDLE_SCOPE_REQUIRED"):
...
def test_new_r04_evidence_is_trusted_and_retained():
    ...
    source = inspect.getsource(fixture._prepare)
    assert "inputs.update(Path(name) for name in R04_EVIDENCE_PINS)" in source
    assert "**R04_EVIDENCE_PINS" in source
    assert "scope.read_bytes(Path(evidence_path), evidence_sha)" in source


def test_r03_calibration_roots_cannot_be_reused(monkeypatch):
    monkeypatch.setenv("MILA_V0224_INSTANCE", "unionv1-d1-u1")
    with pytest.raises(batch.ProviderStop):
        batch.selected_root()
