...
import run_v0224_live_completion as runner
import v0224_live_completion_batch as live
import v0224_live_http as wire
from v0222_presentation_batch_v2 import Batch as OriginalJournal
...
def test_live_keeps_fresh_operations_and_all_original_phase_methods():
    for name in (
        "_operation",
        "_new_scope",
        "authorize",
    ...
    for name in ("launch_once", "_check_journal", "_check_claims"):
        assert getattr(live.Batch, name) is getattr(OriginalJournal, name)
    ...
    assert auth["phase_wall_seconds"] == {"P3": 1800, "P4": 7200}
