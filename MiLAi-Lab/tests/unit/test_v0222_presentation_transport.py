"""Raw transport MockHTTP only; full acceptance and release are the Provider gate."""
...
class FakeBatch:
    def __init__(self, root, episode="p3-01", cap=1):
        self.root, self.events, self.stopped = root, [], None
        self.episode, self.cap = episode, cap
    ...
    def admit(self, episode):
        ...
        return {"cap": self.cap, "deadline": time.time() + 300}
...
def setup(tmp_path, monkeypatch, mode="valid", *, episode="p3-01", cap=1):
    batch = FakeBatch(tmp_path, episode, cap)
    calls = []
    monkeypatch.setattr(
        "v0222_presentation_transport.historical_usage_presentation",
        lambda paths: batch.auth["historical"],
    )
    ...
    provider = Transport(
        tmp_path / "episodes" / episode / "provider",
        batch=batch,
        episode=episode,
...
def test_legal_or_illegal_content_returned_raw_and_settled_for_diagnostic(
    ...
    try:
        provider.verify()
        output = provider.generate("p3-01", body)
...
def test_identity_failure_clears_context_and_prevents_any_later_generation(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr("v0222_presentation_transport.boundary_identity", failed_identity)
...
def test_verify_rechecks_stop_after_identity_attempt_persistence(
    ...
    try:
        with pytest.raises(ProviderStop):
            provider.verify()
        assert len(calls) == expected and provider.context is None
        assert batch.stopped == "STOP_AFTER_IDENTITY_ATTEMPT"
        with pytest.raises(ProviderStop):
            provider.generate("p3-01", body)
        assert len(calls) == expected and not batch.events
    finally:
        provider.close()
...
def test_known_content_edge_cases_are_preserved_not_repaired(tmp_path, monkeypatch, mode):
    ...
    try:
        provider.verify()
        raw = provider.generate("p3-01", body)
...
def test_history_drift_stops_without_tokenize(tmp_path, monkeypatch):
    ...
    try:
        provider.verify()
        monkeypatch.setattr(
            "v0222_presentation_transport.historical_usage_presentation", lambda paths: {}
        )
...
def test_p4_transport_four_requests_each_counted_and_fifth_never_sent(tmp_path, monkeypatch):
    batch, provider, body, calls = setup(tmp_path, monkeypatch, episode="p4-01", cap=4)
    try:
        provider.verify()
        for _ in range(4):
            assert provider.generate("p4-01", body) == '{"text":"x"}'
        cost = usage_state(batch.events)
        assert cost["requests"] == 4 and cost["actual_total_raw_tokens"] == 328
        assert calls.count("/tokenize") == calls.count("/v1/chat/completions") == 4
        assert len({e["request_id"] for e in batch.events}) == 4
        with pytest.raises(ProviderStop, match="EXHAUSTED"):
            provider.generate("p4-01", body)
        assert calls.count("/v1/chat/completions") == 4 and batch.stopped
    finally:
        provider.close()


@pytest.mark.parametrize("cap", [4096.0, True, 4097])
def test_wire_output_cap_has_exact_integer_type_and_value(tmp_path, monkeypatch, cap):
    batch, provider, body, calls = setup(tmp_path, monkeypatch)
    try:
        provider.verify()
        body["max_tokens"] = cap
        with pytest.raises(ProviderStop, match="SEALED_MODEL_PARAMETERS"):
            provider.generate("p3-01", body)
        assert batch.stopped and "/tokenize" not in calls and not batch.events
    finally:
        provider.close()
