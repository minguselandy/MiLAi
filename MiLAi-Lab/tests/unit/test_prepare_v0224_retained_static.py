...
import prepare_v0224_retained_static as controller
import run_v0224_verified_digest_preparation as old_controller
import v0224_retained_static_batch as batch
...
def test_all_six_fixture_children_share_one_authority_and_deadline(parent):
    ...
    assert all("prepare_v0224_retained_static.py" in row["argv"][1] for row in calls[2:])
...
def test_all_old_entries_and_fixed_evidence_preserved():
    ...
    assert len(controller.R04_EVIDENCE_PINS) == 9
    assert (
