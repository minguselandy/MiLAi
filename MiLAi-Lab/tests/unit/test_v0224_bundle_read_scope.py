"""Synthetic R03 authority boundaries; these receipts grant no real execution."""
...
def fixture(tmp_path, *, fresh=False, roles_override=None, receipt_change=None):
    ...
    header = {
        ...
        "roots": roots, "files": files, "blobs": blobs,
    ...
    receipt = {
        ...
        "bundle_sha256": sha(package), "format_revision": header["format_revision"],
        "verifier_files": {str(verifier): sha(verifier.read_bytes())},
        "proof_roots": roots, "fresh_roles": roles,
    ...
    kwargs = {
        "bundle_path": bundle, "bundle_sha256": sha(package),
        "receipt_path": receipt_path, "receipt_sha256": sha(receipt_path.read_bytes()),
        "limits": BundleLimits(max_header_bytes=100_000, max_payload_bytes=100_000,
                               max_paths=100, max_blobs=100),
    }
