"""Prepare one new K1 fixture; not a timing run, K3 seal, or live admission.
...
def prepare():
    ...
    root = selected_root()
    if root.name not in {"k1-u1", "k1-s1", "k1-s2", "k1-u2", "k1-u3", "k1-s3", "k1-prefix"}:
        raise ValueError("ONLY_K1_PREPARATION_NOT_K3_SEAL")
    ...
    with AdmissionReadScope() as scope:
        ...
        entries = [
            ...
            for name in (
                ...
                "prepare_v0223_measurement_fixture.py",
                "run_v0223_admission_pair.py",
                "v0223_reference_prefix.py",
                "run_v0223_calibration.py",
                "inventory_v0223_fixture.py",
            )
        ]
    ...
    result = {
        "status": "K1_FIXTURE_ONLY_NOT_CURRENT_ENGINEERING_OR_K0_PASS",
        "root": str(root),
