"""Assemble two actual K2 inventories for independent review; never grants timing."""
...
def main():
    ...
    from run_v0223_k2_prefix import PAIR_ORDER
    ...
    from v0223_k2_cpu_batch import CPU_BASE
    ...
    for instance, variant in PAIR_ORDER:
        root = CPU_BASE / ("k2v1-" + instance)
    ...
    contract.update(
        status="K2_PAIR_REVIEW_REQUIRED_NOT_TIMING_AUTHORIZED",
        ...
        missing_before_freeze=[
            *contract.get("missing_before_freeze", []),
            "Two actual initial inventories and current SQL independently compared",
            "Candidate source, original/candidate differential "
            "and dynamic negative terminal evidence",
            "Exact pair parent/worker code and lifecycle review; one original then one candidate",
            "Explicit independent K2 freeze review and final status; "
            "engineering and K3 remain future",
        ],
        assembly_source={
            "path": str(LAB / "tools/assemble_v0223_k2_pair.py"),
            "sha256": sha(LAB / "tools/assemble_v0223_k2_pair.py"),
        },
    )
