"""Explicit offline-only root/contract adapter for real-scale CPU replay.
...
Importing creates no instance, process, socket or evidence directory.
"""
...
CPU_ROOT = Path("/cra/memory/mx_memory/evidence/v0222-scoped-cpu/20260912-cold-r1")
CPU_REVISION = "SCOPED_CPU_REPLAY_V1"


def cpu_authorization_source() -> dict:
    return {
        "origin": "USER_DIRECTED_OFFLINE_ENGINEERING",
...
def make_cpu_authorization(root: Path, *, issued: float, expires: float, history: dict) -> dict:
    ...
    if root != CPU_ROOT or root.resolve() != root:
