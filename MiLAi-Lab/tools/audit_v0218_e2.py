"""Offline E2 audit: frozen prompts, exact common A, actual HTTP bytes and world actions."""
...
def audit_prefetch(receipt: dict, initial: dict) -> list[dict]:
    assert receipt["origin"] == "HARNESS_PUBLIC_PREFETCH"
    assert receipt["scope"] == initial["scope"]
    assert receipt["world_snapshot_sha256"] == digest(initial)
    assert receipt["model_calls"] == receipt["business_mutations"] == 0
    responses = [item["response"] for item in receipt["reads"]]
    assert [r["resource"] for r in responses] == ["current", "policy", "records", "history"]
    for response in responses:
        assert response == {
            "resource": response["resource"],
            "version": initial["version"],
            "content": initial[response["resource"]],
            "content_sha256": digest(initial[response["resource"]]),
        }
    return [
        {
            "role": "user",
            "content": json.dumps({"origin": "HARNESS_PUBLIC_PREFETCH", "tool_result": response}),
        }
        for response in responses
    ]
...
def audit(root: Path) -> dict:
    ...
    assert manifest["stage"].startswith("E2_")
    ...
    assert sha(Path(__file__)) == manifest["implementation"]["tools/audit_v0218_e2.py"]
    ...
    for outcome in result["rows"]:
        ...
        prefetch_messages = []
        if phase == "A":
            assert "public_prefetch" not in config
            assert not (directory / "public-prefetch.json").exists()
        else:
            assert config["public_prefetch"] == manifest["public_prefetch"]
            receipt = read(directory / "public-prefetch.json")
            prefetch_messages = audit_prefetch(receipt, initial)
            # Scope is deliberately different; actual presented public responses are identical.
            pair = (key, variant)
            if pair in common_prefetch:
                assert common_prefetch[pair] == prefetch_messages
            common_prefetch[pair] = prefetch_messages
        ...
        for dispatch in dispatches:
            ...
            if prefetch_messages:
                assert messages[2:6] == prefetch_messages
                expected_receipts = [
                    {
                        "origin": "HARNESS_PUBLIC_PREFETCH",
                        "resource": item["response"]["resource"],
                        "version": item["response"]["version"],
                        "content_sha256": item["response"]["content_sha256"],
                    }
                    for item in receipt["reads"]
                ]
                assert dispatch["source_receipts_in_context"][:4] == expected_receipts
        ...
        rows.append(
            {
                ...
                "harness_public_prefetch_reads": len(prefetch_messages),
                "prefetch_message_bytes_per_request": sum(
                    len(m["content"].encode()) for m in prefetch_messages
                ),
                "prefetch_messages_sha256": digest(prefetch_messages),
                "source_reads": dict(
                    Counter(
    ...
    summary = {
        ...
        "harness_public_prefetch_reads": sum(r["harness_public_prefetch_reads"] for r in rows),
        "matched_public_information": True,
        "acquisition_credit": "HARNESS_PREFETCH_IS_NOT_AUTONOMOUS_AGENT_ACQUISITION",
        "shared_A_cost_once": {
    ...
    output = root / "control-audit-v1.json"
    if output.exists():
