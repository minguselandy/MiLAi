...
def _prepare():
    ...
    from v0223_current_cpu_batch import (
        CPU_REVISION,
        OfflineBatch,
    ...
    root = selected_root()
    if root.name.removeprefix("currentv1-") not in {
        "k3-u1",
        "k3-s1",
        "k3-s2",
        "k3-u2",
    ...
    with AdmissionReadScope() as scope:
        ...
        # Retain every frozen K2 source/input/copy, never its mutable stopped SQLite.
        prior_root = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-k2-v1/k2v1-k2-base")
        prior_binding = scope.read_json(
            prior_root / "execution-binding.json",
            "82abff031bb2c76053eb410f2e31c23bcc9457aef050aafcef3a2a64de9a9877",
        )
        ...
        entries = [
            LAB / "tools" / name
            for name in (
                "v0223_current_cpu_batch.py",
                "prepare_v0223_current_fixture.py",
                "inventory_v0223_current_fixture.py",
                "v0223_reference_prefix_v2.py",
                "v0223_coarse_observation.py",
                "v0223_tree_readonly_candidate.py",
                "v0223_transaction_primary_fix.py",
                "run_v0223_current_admission.py",
                "run_v0223_current_calibration.py",
                "assemble_v0223_current_calibration.py",
            )
        ]
        entries.extend(
            LAB / "tests/unit" / name
            for name in ("test_v0223_current_admission.py", "test_v0223_current_calibration.py")
        )
