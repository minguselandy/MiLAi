"""Six frozen cold U/S references; strict U has no internal observer clocks.
...
Calibration is not full Gate A, and no failed position is retried.
"""
...
BASE = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-runtime-union-v1")
ORDER = (
    "unionv1-d1-u1",
    "unionv1-d1-s1",
    "unionv1-d1-s2",
...
REVISION = "V0224_R03_COMPLETE_RUNTIME_UNION_OBSERVATION_01"
LIMITS = {
    "http_requests": 0,
    "model_requests": 0,
    "device_calls": 0,
...
def validate_contract(contract):
    ...
    _unprofiled()
    require(
        type(contract) is dict and contract.get("status") == "R03_RUNTIME_UNION_CALIBRATION_FROZEN",
    ...
    for position in positions:
        ...
        spec = position["first_p4_spec"]
        require(
            type(spec) is dict
            and spec.get("id") == name + "-p4-01"
            and spec.get("stage") == "P4"
            and type(spec.get("actions")) is list
            and len(spec["actions"]) == 1,
            "ORIGINAL_FIRST_SINGLE_WRITE_P4_REQUIRED",
        )
    ...
    ordinary_specs = [
        {
            k: v
            for k, v in p["first_p4_spec"].items()
...
def validate_observation(value, arm):
    require(
        value["mode"] == arm
        and value["callback_counts"] == {"attempted": 9, "returned": 9}
        and value["errors"] == [],
        "COMPLETE_NINE_CALLBACKS_REQUIRED",
    )
    if arm == "U":
        require(
    ...
    require(type(rows) is list and len(rows) == 7, "SEVEN_COMPLETE_RUNTIME_INTERVALS_REQUIRED")
...
def run_reference(position, authority, out, details):
    ...
    from v0224_runtime_union_batch import OfflineBatch, StaticAuthority, selected_root
    ...
    try:
        with installed_transaction_fix():
            batch = OfflineBatch(root, position["binding_sha256"], static_authority=config)
            ...
            require(_exact(spec, position["first_p4_spec"]), "EXACT_ORIGINAL_FIRST_SPEC_REQUIRED")
            ...
            require(
                ...
                and len(rows) == 3
                and [r["turn"] for r in rows] == [1, 2, 3],
                "COMPLETE_THREE_REFERENCE_ROWS_REQUIRED",
            )
            for row in rows:
...
def calibration_verdict(children, workers):
    ...
    pair_ratios = [
        by_position[f"unionv1-d1-s{i}"] / by_position[f"unionv1-d1-u{i}"] for i in (1, 2, 3)
    ]
