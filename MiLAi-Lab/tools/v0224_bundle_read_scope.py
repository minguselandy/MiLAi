"""R03 explicit static authority; original dynamic paths retain physical reads.
...
from v0224_static_bundle import entry_bytes, parse_bundle
...
class BundleReadScope(AdmissionReadScope):
    ...
    def _ensure_authority(self):
        ...
        try:
            ...
            for name, row in roles.items():
                ...
                _digest(row["sha256"])
                if (type(row["roles"]) is not list or not row["roles"]
            ...
            for row in roots:
                ...
                keys.add(row["path"])
            physical = super()._entry(self._bundle_path, self._bundle_sha)
            index = parse_bundle(physical.data, self._limits)
    ...
    def _entry(self, path, expected_sha256):
        ...
        try:
            ...
            fresh = (role is not None or self._physical_depth or canonical in self._promoted
    ...
    def _close(self, body_error=None):
        ...
        if not self._closed:
            ...
            if self._receipt is None:
                ...
                missing = [name for name in self._receipt["fresh_roles"]
                ...
                if missing:
                    errors.append(BundleReadError("FRESH_ROLE_COVERAGE_INCOMPLETE: "
                                                  + repr(sorted(missing))))
            for error in errors:
