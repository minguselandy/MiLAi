"""One inventory-breakdown five-scope U/S diagnostic, never a K3 success run."""
...
def validate_contract(contract, instance, mode):
    ...
    if (
        ...
        or contract.get("status") != "INVENTORY_DIAGNOSTIC_CALIBRATION_FROZEN"
    ):
    ...
    if contract.get("observation_revision") != "INVENTORY_BREAKDOWN_R01":
        raise ValueError("EXACT_INVENTORY_OBSERVATION_REVISION_REQUIRED")
    ...
    for row in order:
        if (
            ...
            or row.get("root") != str(CPU_BASE / ("diagv1-" + row["instance"]))
...
@contextmanager
def installed_diagnostic_stack(observer, *, batch_class):
    ...
    from v0223_inventory_breakdown import installed_inventory_breakdown
    ...
    with (
        ...
        installed_candidate(),
        installed_inventory_breakdown(observer),
...
def observation_coverage(prefix, report, mode):
    ...
    private = (
        sum(
            row["json_docs"] for row in report["records"] if row["category"] == "private_json_parse"
        )
        if mode == "S"
        else None
    )
    return {
        ...
        "observed_private_parse_completed_events": private,
        ...
        "compatibility_gap_meaning": (
            "Gap from public reader only; private stages are now timed separately."
        ),
        "json_parse_copy_meaning": "Public read_json calls only, not all JSON parse/copy work.",
        "private_work": (
            "Private parse, eligibility and copy-guard stages have explicit disjoint "
            "aggregate categories; enclosing inventory exclusive time excludes these stages."
        ),
...
def main():
    ...
    from v0223_diagnostic_cpu_batch import OfflineBatch, selected_root
    ...
    try:
        with (
            installed_diagnostic_stack(observer, batch_class=OfflineBatch),
    ...
    breakdown = inventory_breakdown_report(observer)
    if breakdown["status"] == "INVALID_OBSERVATION" and failure is None:
        failure = RuntimeError("INVENTORY_BREAKDOWN_INVALID_OBSERVATION")
        result["status"] = "INVENTORY_BREAKDOWN_INVALID_OBSERVATION"
