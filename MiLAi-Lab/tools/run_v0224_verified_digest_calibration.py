...
BASE = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-scope-digest-v1")
ORDER = (
    "digestv1-d1-u1",
    "digestv1-d1-s1",
    "digestv1-d1-s2",
...
REVISION = "V0224_R04_SCOPE_VERIFIED_DIGEST_REUSE_01"
LIMITS = {
    "http_requests": 0,
    "model_requests": 0,
    "device_calls": 0,
...
def run_reference(position, authority, out, details):
    ...
    from v0224_verified_digest_batch import OfflineBatch, StaticAuthority, selected_root
...
def calibration_verdict(children, workers):
    ...
    pair_ratios = [
        by_position[f"digestv1-d1-s{i}"] / by_position[f"digestv1-d1-u{i}"] for i in (1, 2, 3)
    ]
