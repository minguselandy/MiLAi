...
def prepare(root: Path) -> dict:
    if root.resolve() != BASE / "v0221/20260911-http-r1-v2":
        raise ValueError("SINGLE_USER_AUTHORIZED_HTTP_BATCH_REQUIRED")
    prior = BASE / "v0221/20260911-http-r1"
    failure = read(prior / "preflight/result.json")
    if (
        failure["reason"] != "UNVALIDATED_CANONICAL_WIRE_PAIR_OR_OPTIONS"
        or failure["model_requests"] != 0
        or failure["tokenize_calls"]
        or (prior / "live-launch.json").exists()
    ):
        raise ValueError("LOCAL_PRE_GENERATION_CORRECTION_ONLY_NOT_A_LIVE_RETRY")
    with sqlite3.connect(f"file:{prior / 'batch.sqlite'}?mode=ro", uri=True) as db:
        if db.execute("SELECT count(*) FROM events").fetchone()[0] != 0:
            raise ValueError("PRIOR_MODEL_ATTEMPT_CANNOT_BE_RESET")
    validate(WIRE)
    validate(OLD)
    w2, w3 = plan_specs()
    ...
    inputs = [
        ...
        *HISTORY,
        ...
        prior / "preflight/result.json",
        prior / "execution-binding.json",
    ]
    ...
    contract = {
        ...
        "preflight_revision": "TOP_LEVEL_ANYOF_COVERAGE_MATCH_V2_NO_LIVE_RETRY",
        "prior_preflight_model_requests": 0,
    }
    entries = [
        LAB / "tools" / n
        for n in (
            "prepare_v0221_http_v2.py",
            "preflight_v0221_http_v2.py",
            "run_v0221_http_v2.py",
            "v0221_http_worker_v2.py",
            "v0221_http_audit.py",
        )
    ]
    entries.append(LAB / "tests/unit/test_v0221_http.py")
    entries.append(LAB / "tests/unit/test_v0221_http_admission_v2.py")
