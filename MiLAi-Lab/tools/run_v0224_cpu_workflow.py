...
BASE = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-workflow-v1")
ORDER = (
    "flowv1-d1-u1",
    "flowv1-d1-s1",
    "flowv1-d1-s2",
...
REVISION = "V0224_R04_CPU_WORKFLOW_01"
LIMITS = {
    "http_requests": 0,
    "model_requests": 0,
    "device_calls": 0,
...
def validate_contract(contract):
    ...
    _unprofiled()
    require(
        type(contract) is dict and contract.get("status") == "R04_LONG_PATH_CALIBRATION_FROZEN",
...
def calibration_verdict(children, workers):
    ...
    pair_ratios = [
        by_position[f"flowv1-d1-s{i}"] / by_position[f"flowv1-d1-u{i}"] for i in (1, 2, 3)
    ]
