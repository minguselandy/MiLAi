"""Prepare one new paired K2 fixture; not a timing run, K3 seal, or live admission.
...
def _prepare():
    ...
    from v0223_k2_cpu_batch import (
        CPU_REVISION,
        OfflineBatch,
    ...
    root = selected_root()
    if root.name.removeprefix("k2v1-") not in {"k2-base", "k2-edit"}:
        raise ValueError("ONLY_K2_PAIRED_PREPARATION_NOT_K3_SEAL")
    ...
    with AdmissionReadScope() as scope:
        ...
        # Retain the K1 revision's source/input/copy burden as well as V0222.
        prior_root = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-v2/v2-k1-u1")
        prior_binding = scope.read_json(
            prior_root / "execution-binding.json",
            "d3ec1135b2e0f23bde78f7f4dc01f9bb42baadd8ade7b155a26138670e70bf3a",
        )
        ...
        entries = [
            LAB / "tools" / name
            for name in (
                "v0223_k2_cpu_batch.py",
                "prepare_v0223_k2_fixture.py",
                "inventory_v0223_k2_fixture.py",
    ...
    result = {
        "status": "K2_FIXTURE_ONLY_NOT_CURRENT_ENGINEERING_OR_K0_PASS",
        "root": str(root),
