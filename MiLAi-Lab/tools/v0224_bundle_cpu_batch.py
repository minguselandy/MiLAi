"""Explicit R03 synthetic CPU binding; no original modules are patched.
...
from v0224_bundle_read_scope import BundleReadScope, _canonical, _digest
from v0224_static_bundle import BundleLimits

# Fixed prospective instances, never selected by model input.
CPU_BASE = Path("/cra/memory/mx_memory/evidence/v0224/20260913-gate-a-bundle-v1")
INSTANCE_NAMES = frozenset({"bundle-v1-slice"})
CPU_REVISION = "V0224_BUNDLE_AUTHORITY_CPU_V1"
...
class StaticAuthority:
    ...
    def __post_init__(self):
        if type(self.limits) is not BundleLimits:
            raise ProviderStop("EXACT_IMMUTABLE_BUNDLE_LIMITS_REQUIRED")
        # Pure configuration validation only; no IO or expected-pin discovery.
        _canonical(str(self.bundle_path))
        _canonical(str(self.receipt_path))
        _digest(self.bundle_sha256)
        _digest(self.receipt_sha256)
        if self.bundle_path == self.receipt_path:
            raise ProviderStop("DISTINCT_EXTERNAL_AUTHORITY_FILES_REQUIRED")
        if not isinstance(self.bundle_path, Path) or not isinstance(self.receipt_path, Path):
            raise ProviderStop("EXPLICIT_AUTHORITY_PATHS_REQUIRED")

    def scope_arguments(self):
        return {
