...
def main():
    ...
    from run_v0223_diagnostic_admission import validate_contract
    ...
    if sha(args.contract) != args.contract_sha256:
        raise ValueError("DIAGNOSTIC_CALIBRATION_CONTRACT_HASH_MISMATCH")
    contract = json.loads(args.contract.read_bytes())
    validate_contract(contract, "d1-u1", "U")
    for name, digest in contract["files"].items():
        if sha(Path(name)) != digest:
            raise ValueError("DIAGNOSTIC_CALIBRATION_EXECUTABLE_OR_INPUT_DRIFT")
    ...
    for position in contract["calibration_order"]:
        ...
        try:
            child = run_child(
                [
                    sys.executable,
                    str(LAB / "tools/run_v0223_diagnostic_admission.py"),
                    "--contract",
                    str(args.contract),
                    "--contract-sha256",
    ...
    save(
        out / "terminal.json",
        {
            ...
            if complete
            else "DIAGNOSTIC_CALIBRATION_STOP",
            "rows": rows,
            "failures": failures,
