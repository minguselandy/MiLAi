...
[Independent Formation R1](docs/MILAI_LIFECYCLE_V24_FORMATION_R1_RESULTS_20260927.md)
finishes four small cases per arm: B1 and the prospective cue both retain 0/2 future items,
while temporary controls pass. The cue is delivered but adds 703 tokens without benefit.
Preserve this failure before a minimal generic instruction repair; no extra model phase
...
No arm passes all three. The adapter, 27 narrow checks and one package build are complete;
vLLM settings and historical results remain unchanged. Costs: 45 generations / 41457 tokens /
808 embedding tokens and 9 exact reads, including 3 reads of irrelevant Y. See
[results](docs/MILA_FRESHNESS_PROJECTION_V19_REPAIR_RESULTS_20260926.md) and
[reproduction](docs/MILA_FRESHNESS_PROJECTION_V19_REPAIR_REPRODUCTION_20260926.md).

Previous scoped delivery: [ODR Goal v19](docs/MILA_ON_DEMAND_RECONSTRUCTION_GOAL_v19.md),
