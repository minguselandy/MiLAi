#!/usr/bin/env python3
"""Run focused implementation quality gates for DG-25 S4B readiness."""
...
from evals.dg25.s4b_readiness import build_s4b_source_manifest, file_identity
...
def run_quality(*, run_id: str, root: Path) -> int:
    ...
    if output.is_symlink() or output.exists():
        raise FileExistsError("DG25_S4B_QUALITY_EXISTING_OUTPUT_REJECTED")
    _validate_protected_diffs(root)
    before = build_s4b_source_manifest(root)
    ...
    try:
        for gate_id, argv in _gate_vectors(root):
            ...
            if result.returncode != 0:
                raise RuntimeError(f"DG25_S4B_QUALITY_GATE_FAILED:{gate_id}")

        _validate_protected_diffs(root)
        after = build_s4b_source_manifest(root)
        if before["files"] != after["files"]:
            raise ValueError("DG25_S4B_SOURCE_CHANGED_DURING_QUALITY_GATES")
        receipt: dict[str, Any] = {
            "schema": "milai.dg25.s4b-quality-receipt.v0.1",
            "run_id": run_id,
            "status": "PASS_DG25_S4B_QUALITY",
            "gates": gates,
        ...
        _write_json(
            ...
            {
                "schema": "milai.dg25.s4b-quality-failure.v0.1",
                "run_id": run_id,
                "gates": gates,
                "automatic_retries": 0,
            },
...
def _gate_vectors(root: Path) -> list[tuple[str, list[str]]]:
    ...
    lint_sources = [*typed_sources, "tests/test_dg25_s4b.py"]
    expanded_tests = [
    ...
    return [
        ("targeted-pytest", [pytest, "-q", "tests/test_dg25_s4b.py"]),
        ("expanded-dg25-pytest", [pytest, "-q", *expanded_tests]),
        ("strict-mypy", [mypy, "--strict", *typed_sources]),
        ("ruff", [ruff, "check", *lint_sources]),
        (
            "compileall",
            [python, "-m", "compileall", "-q", *typed_sources],
        ),
        (
...
def _validate_protected_diffs(root: Path) -> None:
    for path, expected in PROTECTED_DIFF_DIGESTS.items():
        result = subprocess.run(
            ["git", "diff", "--", path], cwd=root, check=True, capture_output=True
        )
        if hashlib.sha256(result.stdout).hexdigest() != expected:
            raise ValueError(f"DG25_S4B_PROTECTED_USER_DIFF_DRIFT:{path}")
...
def _validated_output(root: Path, run_id: str) -> Path:
    if run_id in {".", ".."} or SAFE_COMPONENT.fullmatch(run_id) is None:
        raise ValueError("DG25_S4B_QUALITY_RUN_ID_NOT_SAFE")
    parent = root / QUALITY_ROOT
    if parent.resolve(strict=False) != parent.absolute():
        raise ValueError("DG25_S4B_QUALITY_ROOT_SYMLINK_OR_DRIFT")
    output = parent / run_id
    if output.absolute().parent != parent.absolute():
        raise ValueError("DG25_S4B_QUALITY_OUTPUT_NOT_DIRECT_CHILD")
    return output
