...
SOURCE = ROOT / "docs/reviews/DG-10-r0-r2-ai-audit-revise-candidate.4.6-2026-08-22.json"
SOURCE_SHA256 = "e0cf1999fe6ae46998008b8e661f842e61246c70123874229f37ed2b8f84cded"
DEFAULT_OUTPUT = ROOT / (
    "docs/reviews/DG-10-candidate4-ai-finding-register-materialized-"
    "candidate.4.6-2026-08-22.json"
)
FINDING_IDS = (
    "DG10-C4-AI-024",
    "DG10-C3-AI-007",
...
def build_register() -> dict[str, Any]:
    if (
        ...
        raise remediation.RemediationError("candidate.4.6 AI review receipt drift")
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    review = source.get("review_result")
    if not isinstance(review, dict) or not isinstance(review.get("findings"), list):
        raise remediation.RemediationError("candidate.4.6 finding register absent")
    ...
    if tuple(item.get("finding_id") for item in findings) != FINDING_IDS:
        raise remediation.RemediationError("candidate.4.6 finding ID or denominator drift")
    return {
        "schema": "milai.dg10.parent-ai-finding-register-materialization.v1",
        "candidate_id": remediation.CANDIDATE,
        "parent_identity_suffix": "candidate.4.6",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Materialize candidate.4.6 AI finding semantics")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    args = parser.parse_args()
    output = args.output.absolute()
    remediation.atomic_write_new(output, remediation.encoded_json(build_register()))
    print(json.dumps({"output": str(output), "finding_count": len(FINDING_IDS)}, sort_keys=True))
