...
SOURCE = ROOT / "docs/reviews/DG-10-r0-r2-ai-audit-revise-candidate.4.4-2026-08-22.json"
SOURCE_SHA256 = "bbb04dcbb83aaef5e7ebb87e48f78d63511b95ff6601d3de3146f7d87951588d"
DEFAULT_OUTPUT = ROOT / (
    "docs/reviews/DG-10-candidate4-ai-finding-register-materialized-"
    "candidate.4.4-2026-08-22.json"
)
FINDING_IDS = (
    "DG10-C4-AI-015",
    "DG10-C4-AI-019",
...
def build_register() -> dict[str, Any]:
    if remediation.sha256_file(SOURCE) != SOURCE_SHA256:
        raise remediation.RemediationError("candidate.4.4 AI review receipt drift")
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    review = source.get("review_result")
    if not isinstance(review, dict) or not isinstance(review.get("findings"), list):
        raise remediation.RemediationError("candidate.4.4 finding register absent")
    ...
    if tuple(item.get("finding_id") for item in findings) != FINDING_IDS:
        raise remediation.RemediationError("candidate.4.4 finding ID or denominator drift")
    return {
        "schema": "milai.dg10.parent-ai-finding-register-materialization.v1",
        "candidate_id": remediation.CANDIDATE,
        "parent_identity_suffix": "candidate.4.4",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Materialize candidate.4.4 AI finding semantics")
