...
OLD_SUFFIX = "candidate.4.13-2026-08-22.json"
NEW_SUFFIX = "candidate.4.14-2026-08-22.json"
REVISE = ROOT / (
    "docs/reviews/DG-10-r0-r2-ai-audit-revise-candidate.4.10-2026-08-22.json"
)
REVISE_SHA256 = "2e43ae742040abdda53f1c32fd80668c6082bb5eb07f4663218fc2b93d980fd3"
DEFAULT_OUTPUT = ROOT / (
...
def build_receipt() -> dict[str, Any]:
    if remediation.sha256_file(REVISE) != REVISE_SHA256:
        raise remediation.RemediationError("candidate.4.10 revise receipt hash drift")
    ...
    if not (
        ...
        and revise.get("attempt_id") == "candidate.4-primary-012"
        and revise.get("status")
        == "AI_AUDIT_REVISE_POSTHOC_ATTESTATION_FORGEABLE_FAIL_CLOSED"
        and revise.get("reasoning_effort") == "xhigh"
        and result.get("open_p0_count") == 3
        and result.get("open_p1_count") == 2
        and result.get("open_p2_count") == 0
    ):
        raise remediation.RemediationError("candidate.4.10 revise receipt drift")
    old = _identity_set(OLD_SUFFIX)
    old["ai_revise_receipt_sha256"] = REVISE_SHA256
    ...
    return {
        ...
        "remediated_audit_attempt": "candidate.4-primary-012",
        "reason_codes": [
            "AI_RUNS_REQUIRE_SEPARATE_UID_ED25519_SIGNED_LIVE_EXECUTION",
            "AUTHORITY_PRIVATE_KEY_IS_EXTERNAL_AND_UNREADABLE_TO_WORKSPACE_AUTHOR",
            "UNSIGNED_V1_AND_POSTHOC_ATTEMPT_DIRECTORIES_FAIL_CLOSED",
            "TEST_ACCESS_AND_R3_REPLAY_THE_AUTHORITY_SIGNATURE",
            "FIXED_AUTHORITY_OWNED_ATTEMPT_ROOTS_ARE_REQUIRED",
        ],
        ...
        "active_identity_suffix": "candidate.4.14",
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze candidate.4.14 identity supersession"
    )
