#!/usr/bin/env python3
"""Build a fresh DG-25 S4A package for independent authorization review."""
...
def run_readiness(
    *,
    ...
    root = root.resolve()
    output = _validated_output_path(root, run_id)
    quality_path = _validated_quality_path(root, quality_receipt)
    if output.is_symlink() or output.exists():
        raise FileExistsError("DG25_S4A_READINESS_EXISTING_OUTPUT_REJECTED")

    materials = build_s4a_readiness_materials(
        root=root,
        run_id=run_id,
    ...
    try:
        ...
        receipt: dict[str, Any] = {
            "schema": "milai.dg25.s4a-readiness-receipt.v0.1",
            "run_id": run_id,
            "status": (
            ...
            "historical_readiness_receipt": file_identity(
                root, root / FIXED_INPUT_PATHS["historical_readiness_receipt"]
            ),
            ...
            "authorization": {
                "s4a_e2_label_free_generation": False,
                "e2_all_arm_seal": False,
                "combined_all_arm_seal": False,
                "labels_or_registry_content": False,
        ...
        json.dumps(
            {
                ...
                "authorization_bindings_digest": receipt["authorization_request"][
...
def _validated_output_path(root: Path, run_id: str) -> Path:
    if run_id in {".", ".."} or SAFE_COMPONENT.fullmatch(run_id) is None:
        raise ValueError("DG25_S4A_READINESS_RUN_ID_NOT_SAFE")
    parent = root / S4A_READINESS_ROOT
    if parent.resolve(strict=False) != parent.absolute():
        raise ValueError("DG25_S4A_READINESS_ROOT_SYMLINK_OR_DRIFT")
    output = parent / run_id
    if output.absolute().parent != parent.absolute():
        raise ValueError("DG25_S4A_READINESS_OUTPUT_NOT_DIRECT_CHILD")
    return output
...
def _validated_quality_path(root: Path, requested: Path) -> Path:
    if requested.is_absolute() or not requested.parts or ".." in requested.parts:
        raise ValueError("DG25_S4A_QUALITY_RECEIPT_PATH_INVALID")
    quality_root = (root / QUALITY_ROOT).resolve(strict=False)
    path = root / requested
    if path.is_symlink() or not path.is_file():
        raise ValueError("DG25_S4A_QUALITY_RECEIPT_NOT_REGULAR_FILE")
    try:
        path.resolve(strict=True).relative_to(quality_root)
    except ValueError as exc:
        raise ValueError("DG25_S4A_QUALITY_RECEIPT_OUTSIDE_QUALITY_ROOT") from exc
    return path
