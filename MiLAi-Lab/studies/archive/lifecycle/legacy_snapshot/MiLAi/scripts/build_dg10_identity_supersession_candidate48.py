...
OLD_SUFFIX = "candidate.4.11-2026-08-22.json"
NEW_SUFFIX = "candidate.4.12-2026-08-22.json"
REVISE = ROOT / (
...
def build_receipt() -> dict[str, Any]:
    ...
    if not (
        ...
        and revise.get("attempt_id") == "candidate.4-primary-010"
        and revise.get("status") == "AI_AUDIT_REVISE"
        and revise.get("reasoning_effort") == "xhigh"
        and result.get("open_p0_count") == 0
        and result.get("open_p1_count") == 1
    ):
        raise remediation.RemediationError("candidate.4.8 revise receipt drift")
    ...
    return {
        ...
        "remediated_audit_attempt": "candidate.4-primary-010",
        "reason_codes": [
            "ALL_65_RUNTIME_WHEEL_PAYLOAD_FILES_HAVE_MATERIALIZED_SOURCE_BYTES",
            "R2_RUNNER_CLOSURE_INCLUDES_RUNTIME_NON_PYTHON_PACKAGE_DATA",
            "ACTIVE_SOURCE_INVENTORY_INCLUDES_RUNTIME_NON_PYTHON_PACKAGE_DATA",
            "WHEEL_AND_SOURCE_PATH_SET_AND_BYTES_ARE_TESTED_EQUAL",
            "FRESH_WHEEL_REBUILD_USES_THE_MATERIALIZED_CLOSED_SOURCE_TREE",
        ],
        ...
        "active_identity_suffix": "candidate.4.12",
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze candidate.4.12 identity supersession"
    )
