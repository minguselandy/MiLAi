...
OLD_SUFFIX = "candidate.4.17-2026-08-22.json"
NEW_SUFFIX = "candidate.4.18-2026-08-22.json"
...
def build_receipt() -> dict[str, Any]:
    if remediation.sha256_file(EXECUTION_FAILURE) != EXECUTION_FAILURE_SHA256:
        raise remediation.RemediationError("candidate.4.13 execution failure drift")
    ...
    if not (
        ...
        and failure.get("attempt_id") == "candidate.4-primary-015"
        and failure.get("reason_code")
        == "DUPLICATE_KNOWN_MODEL_REFRESH_STDERR_DIAGNOSTIC"
    ):
        raise remediation.RemediationError(
            "candidate.4.13 execution failure semantics drift"
        )
    ...
    return {
        ...
        "status": "SUPERSEDED_AFTER_STATIC_MODEL_CATALOG_HARDENING_BEFORE_ACCEPTANCE",
        "superseded_identity_set": old,
        "active_identity_set": current,
        "remediated_audit_attempt": "candidate.4-primary-015",
        "reason_codes": [
            "FREEZE_ROOT_OWNED_READ_ONLY_STATIC_CODEX_MODEL_CATALOG",
            "BIND_MODEL_CATALOG_PATH_HASH_UID_GID_AND_MODE_IN_AUTHORITY_POLICY",
            "INJECT_MODEL_CATALOG_INTO_EXACT_SIGNED_CODEX_COMMAND",
            "SIGN_AND_REPLAY_MODEL_CATALOG_RUNTIME_OBSERVATIONS",
            "PREVENT_PERIODIC_REMOTE_MODEL_LIST_REFRESH",
            "KEEP_STRICT_STDERR_FAIL_CLOSED_CLASSIFIER",
        ...
        "active_identity_suffix": "candidate.4.18",
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze candidate.4.18 static-catalog identity supersession"
    )
