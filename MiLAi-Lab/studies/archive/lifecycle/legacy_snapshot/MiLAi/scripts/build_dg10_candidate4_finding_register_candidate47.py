...
SOURCE_SHA256 = "207c38d16c27ce347f47be09f0ec05e98e012488e33e08c422018cf545a97612"
DEFAULT_OUTPUT = ROOT / (
    "docs/reviews/DG-10-candidate4-ai-finding-register-materialized-"
    "candidate.4.7-2026-08-22.json"
)
FINDING_IDS = ("DG10-C3-AI-005", "DG10-C4-AI-021")
...
def build_register() -> dict[str, Any]:
    if (
        ...
        raise remediation.RemediationError("candidate.4.7 AI review receipt drift")
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    if source.get("status") != "AI_AUDIT_REVISE_CLOSED_SET_REJECTED_FAIL_CLOSED":
        raise remediation.RemediationError("candidate.4.7 fail-closed status drift")
    review = source.get("review_result")
    if not isinstance(review, dict) or not isinstance(review.get("findings"), list):
        raise remediation.RemediationError("candidate.4.7 finding register absent")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Materialize candidate.4.7 AI finding semantics"
    )
