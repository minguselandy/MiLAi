...
OLD_SUFFIX = "candidate.4.10-2026-08-22.json"
NEW_SUFFIX = "candidate.4.11-2026-08-22.json"
REVISE = ROOT / (
...
def build_receipt() -> dict[str, Any]:
    ...
    if not (
        ...
        and revise.get("attempt_id") == "candidate.4-primary-009"
        and revise.get("status")
        == "AI_AUDIT_REVISE_CLOSED_SET_REJECTED_FAIL_CLOSED"
        and revise.get("reasoning_effort") == "xhigh"
        and result.get("open_p0_count") == 0
        and result.get("open_p1_count") == 2
    ):
        raise remediation.RemediationError("candidate.4.7 revise receipt drift")
    ...
    return {
        ...
        "remediated_audit_attempt": "candidate.4-primary-009",
        "reason_codes": [
            "ATOMIC_PER_SLOT_PROVIDER_CLAIM_PRECEDES_NETWORK_IO",
            "PROVIDER_CLAIM_IS_FSYNC_JOURNALED_BEFORE_NETWORK_OPEN",
            "RAW_RESPONSE_IS_FSYNC_RETAINED_BEFORE_JSON_PARSE",
            "PROVIDER_NATIVE_ID_IS_JOURNALED_BEFORE_SEMANTIC_VALIDATION",
            "DIRECT_REPEATED_AND_CONCURRENT_SLOT_REPLAY_FAILS_BEFORE_NETWORK",
            "FIXED_LEDGER_FAILURE_LATCH_CLAIM_AND_RAW_PATHS_ARE_BOUND",
            "AI_PROCESS_TIMEOUT_AND_TERMINATION_METADATA_ARE_VALIDATED",
            "HISTORICAL_UNBOUND_AI_CITATION_REMAINS_FAIL_CLOSED",
        ],
        ...
        "active_identity_suffix": "candidate.4.11",
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze candidate.4.11 identity supersession"
    )
