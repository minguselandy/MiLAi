"""Run Candidate R in a fresh database and persist its unscored checkpoint."""
...
MARKER = "ML_CLOSURE_CANDIDATE_R_RESULT="


def execute(*, output_path: Path = OUTPUT) -> dict[str, Any]:
    if output_path.exists():
        raise RuntimeError("CANDIDATE_R_OUTPUT_ALREADY_EXISTS")
    ...
    database = f"milai_smoke_mlr_{secrets.token_hex(8)}"
    created = False
    ...
    try:
        ...
        completed = subprocess.run(
            [
                ...
                str(Path(__file__).with_name("candidate_r_acquisition.py")),
            ],
            cwd=RUNTIME,
            env=environment,
            text=True,
    ...
    if completed is None:
        raise RuntimeError("CANDIDATE_R_NOT_EXECUTED")
    ...
    result["status"] = (
        "PASS_C0_CANDIDATE_R_REPRODUCTION"
        if completed.returncode == 0
        and result.get("baseline_replay_semantically_identical") is True
    ...
    for line in reversed(stdout.splitlines()):
        if line.startswith(MARKER):
            ...
            if isinstance(value, dict):
                return value
    raise RuntimeError("CANDIDATE_R_RESULT_MARKER_MISSING")
