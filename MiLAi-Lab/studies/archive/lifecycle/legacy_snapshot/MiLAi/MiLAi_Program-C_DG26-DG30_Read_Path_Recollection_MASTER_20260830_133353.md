...
> 文档版本：1.1 / ACTIVE PROGRAM-C SUBPLAN
...
> 当前局部入口（由 MILA-ML-MASTER 授权）：DG-27
...
| DG-27 | pre-execution lock only | 尚无 effect run 或 terminal | 作为当前入口 |
| DG-28～DG-30 | 未执行 | 无效果证据 | 按 MILA-ML-MASTER 与本地 Goal 条件进入 |
...
DG-27
  ↓ DG27-H1 repaired
DG-28 minimal deterministic union
...
2. 失败 case 继续完成整组 run；只有权限泄漏、Canonical mutation 或数据损坏立即停止。
