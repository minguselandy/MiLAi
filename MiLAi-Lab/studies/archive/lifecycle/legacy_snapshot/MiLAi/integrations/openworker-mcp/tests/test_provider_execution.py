...
@pytest.mark.parametrize(
    ("mode", "reason_code", "request_started", "fixture_attempts"),
    [
        ("DOWN", "PROVIDER_UNAVAILABLE", False, 0),
        ("MALFORMED", "PROVIDER_STREAM_FAILED", True, 1),
        ("TIMEOUT", "PROVIDER_UNAVAILABLE", False, 1),
    ],
)
def test_real_provider_fault_fixture_stream_terminal_is_exact_and_zero_retry(
    tmp_path: Path,
    ...
    fixture_ledger_path = tmp_path / f"fixture-{mode.casefold()}.json"
    fixture_context = (
        nullcontext(None)
        if mode == "DOWN"
        else ProviderFaultInjector(
            mode,  # type: ignore[arg-type]
            ledger_path=fixture_ledger_path,
            timeout_delay_seconds=0.15,
        )
    )
    with fixture_context as fixture:
        endpoint = _closed_loopback_endpoint() if fixture is None else fixture.base_url
        current = _request(f"logical-stream-{mode.casefold()}", "stream")
        request = ProviderRequest(
            logical_request_id=current.logical_request_id,
            transport=current.transport,
            payload={**current.payload, "stream_options": {"include_usage": True}},
            prompt_token_budget=current.prompt_token_budget,
            completion_token_budget=current.completion_token_budget,
            timeout_seconds=0.02 if mode == "TIMEOUT" else 0.2,
        )
        gateway = ProviderExecutionGateway(
            _manifest(
                tmp_path / f"manifest-{mode.casefold()}.json",
                endpoint_identity=endpoint,
            ),
            tmp_path / f"gateway-{mode.casefold()}.jsonl",
        )

        with pytest.raises(ProviderCallError, match=f"^{reason_code}$"):
            b"".join(gateway.execute_stream(request, StreamCompletionTransport()))
        if fixture is not None:
            assert fixture.native_attempts == fixture_attempts
    ...
    assert sum(event["event"] == "RESERVED" for event in events) == 1
    assert not any(event["event"] == "POST_PROVIDER_TERMINAL" for event in events)
    if mode != "DOWN":
        fixture_receipt = json.loads(fixture_ledger_path.read_text(encoding="utf-8"))
        assert fixture_receipt["native_attempts"] == fixture_attempts
        assert fixture_receipt["automatic_retries"] == 0
        assert fixture_receipt["raw_request_persisted"] is False
        assert fixture_receipt["request_credentials_persisted"] is False
        assert "synthetic" not in fixture_ledger_path.read_text(encoding="utf-8")
