...
version: "0.3"
status: S2_AUTHORIZED_PENDING_TRACE_EQUIVALENCE
...
```text
run-lock: var/mf01/run-lock.json
sha256: df86fac939d8033d3bd97a1add75b527b425080498242ab17510031a61c3a9d1
S0: SEALED
S1: SEALED
cases: 14
labels: 125
obligation kinds: 10/10 adequate
scope coverage: 1.0
determinability: 1.0
label seal: c2aa1d048fb0da87d6cce79ef4753f3ddf5d4eae5510bd2f6f0a215fa12717a8
```

旧 run-lock 中的 `S0_S1_ONLY` 反映创建时 Master 授权；MILA-ML-MASTER@1.2 现已授权 S2–S4。不改写旧 lock，在 trace/results 中引用新 Master identity。
...
- Entry：Master 授权已满足；S1 label adequacy 通过；Evidence snapshot 与产品行为基线已封存；剩余唯一 entry gate 是 trace hook behavior-equivalence smoke。
...
1. 实现最小 passive trace hook，不增加 Provider/embedding 调用。
2. 运行 behavior-equivalence smoke；若发现行为变化，修 hook 后重做 smoke，不终止 Formation Program。
3. 在已封存 14 cases / 125 labels 上并行产生 `trace.jsonl`。
4. 执行 first-loss scorer，写 `results.json` 和 `terminal.json`。
5. 封存结果后选择 MF-02、MF-03 或 MF-04；MF-01 内不修 extractor。

运行资源：CPU lane 与 DG-27 GPU lane 并行；可按 case 分片到最多 8 workers，但最终以 obligation identity 确定性合并。不为 MF-01 占用生成模型服务。
