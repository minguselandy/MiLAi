...
> 文档版本：0.2 / ACTIVE PROGRAM-A SUBPLAN
...
> 当前入口：[MF-01 Formation First-Loss Audit](./MiLAi_MF-01_Formation_First-Loss_Audit_GOALS.md) 的 S0～S1 设计
...
1. 当前只执行 MF-01 的 S0～S1 schema、label 与 trace 设计；S2 effect 只等待 MF-01 自身 label adequacy、snapshot seal 与 behavior-equivalence entry gate，不等待 DG-28。
2. 每个后继 Goal 只在 MF-01 对应 first-loss 证据支持时创建；编号不形成串行前置。
3. 开发实验允许中间错误，不因单一 metric miss 停止整组诊断。
