"""Mechanical, scoped source acquisition; semantic readiness remains the Host's decision."""
...
class SourceRef:
    source_id: str
    version: str
    kind: str = "SOURCE_FILES"
    eligible: bool = True
    entrypoint: str = "source_read"
    size_bytes: int | None = None
    searchable: bool = True
    product_populated: bool | None = False
...
class AcquisitionHelper:
    ...
    async def read_source(self, binding: Binding, ref: SourceRef, cursor: int = 0) -> SourcePage:
        ...
        self.calls[rid] = Acquisition(page, queue, time.monotonic() - start,
                                      received_bytes)
        return page
    ...
    async def presentation(self, binding: Binding, request_ids: list[str],
        ...
        """Recheck current disclosure for every send, including previously acquired pages."""
    ...
    def record_presentation(self, model_request_id: str, pages: list[SourcePage]) -> None:
        """Call only after the transport reports the actual payload sent, not on assembly."""
        for page in pages:
            if page.text:
