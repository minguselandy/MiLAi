...
| Gate | Candidate status | Meaning |
| --- | --- | --- |
| AF-00 Baseline | PASS_CANDIDATE | inputs and local dependency identity captured |
| AF-01 Objects | PASS_CANDIDATE | object/owner/mutability catalog present |
| AF-02 Invariants | PASS_CANDIDATE | machine crosswalk present and validated |
| AF-03 Permissions | PASS_CANDIDATE | role/grant/RLS contract and current tests mapped |
| AF-04 Transactions | PASS_CANDIDATE | TX/EP/CAS/rollback/outbox mapped |
| AF-05 Retrieval/Context | PASS_CANDIDATE | QueryPlan/Gate/protected context/trace mapped |
| AF-06 Delete/Recovery | PASS_CANDIDATE | sync block, purge, backup and restore mapped |
| AF-07 Isolation | PASS_CANDIDATE | external/model credential boundary mapped |
| AF-08 Operations | PASS_CANDIDATE | runbooks/reports/manifest evidence mapped |
| AF-09 Lock Review | PENDING_INDEPENDENT_REVIEW | candidate.4 rereview REVISE；F01/exact F11 closed，F12 candidate.5 remediation awaiting the same independent reviewer |

`PASS_CANDIDATE` 表示候选材料与自动证据齐备，不等于架构已冻结。只有 AF-09 独立评审接受并签署
后，版本才可从 candidate 转为 `1.0.0 FROZEN`。

## 5. Usage

```bash
python architecture/v1.0-candidate/scripts/validate_bundle.py
python architecture/v1.0-candidate/scripts/verify_lock.py --scope all --mode development
python -m unittest discover -s architecture/v1.0-candidate/tests -v
```
