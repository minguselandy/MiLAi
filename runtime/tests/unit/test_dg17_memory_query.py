...
@pytest.mark.parametrize(
    ("query", "family"),
    [
        ("What kitchen appliance did I buy 10 days ago?", "TEMPORAL_FILTER"),
        (
...
def test_q3a_unsupported_operator_is_typed_ambiguous_not_guessed() -> None:
    ir = MemoryQueryCompiler().compile(
        "What was the average percentage across every memory?",
        reference_time=REFERENCE,
    )

    assert ir.mode == "AMBIGUOUS"
    assert ir.steps == []
    assert ir.requirements == []
    assert ir.planner_trace.source == "AMBIGUOUS"
    assert ir.planner_trace.auxiliary_model_calls == 0
...
def test_q3a_v01_translation_is_explicit_deterministic_and_receiptable() -> None:
    compiler = MemoryQueryCompiler()
    ...
    assert translated.model_dump(exclude={"planner_trace"}) == product.model_dump(
        exclude={"planner_trace"}
    )
