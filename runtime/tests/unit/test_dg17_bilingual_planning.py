...
def test_q3b_first_operator_batch_has_english_and_chinese_plans(
    ...
    for query in (english, chinese):
        ...
        assert all(requirement.entity_constraints for requirement in plan.requirements)
...
def test_q3b_unsafe_generic_lookup_fallback_is_typed_ambiguous(query: str) -> None:
    plan = MemoryQueryCompiler().compile(query, reference_time=REFERENCE)

    assert plan.mode == "AMBIGUOUS"
    assert plan.steps == []
    assert plan.requirements == []
    assert plan.completeness == "UNSTRUCTURED_EVIDENCE_ALLOWED"
    assert plan.planner_trace.source == "AMBIGUOUS"
    assert infer_operator_family(plan) is None
