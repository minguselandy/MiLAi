...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Could this affect the release?", "POSSIBLE", "SEARCH", None),
...
def test_runtime_query_interpreter_is_task_free_and_language_aware(
    ...
    assert result.interpreter_version == "runtime-query-interpreter-v1"
...
def test_runtime_query_interpreter_applies_invocation_floor_without_task_truth() -> None:
    ...
    assert (explicit.intent, explicit.requirement) == ("POSSIBLE", "SEARCH")
...
def test_resolve_uses_one_l1_retrieval_and_adapts_hit_without_task() -> None:
    ...
    # The reranker budget is an upper bound and must never exceed the candidate pool.
    assert access_plan.reranker_candidate_cap == min(20, access_plan.candidate_cap) == 12
...
def test_nonformation_handoff_preserves_frozen_result_projection() -> None:
    ...
    assert "_reader_evidence_boundary" not in outcome
