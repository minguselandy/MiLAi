...
def test_full_forward_migration_chain_on_empty_database(
    ...
    try:
        ...
        assert revision == ("0049_cleanup_terminal_counts",)
        with pytest.raises(
...
def test_formation_hydration_migration_is_reversible_and_least_privilege(
    ...
    try:
        ...
        assert reupgraded == ("0049_cleanup_terminal_counts", True)
    finally:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            admin.execute(
                sql.SQL("DROP DATABASE IF EXISTS {} WITH (FORCE)").format(
...
def test_dg17_structured_speaker_migration_round_trip_preserves_unknown_history(
    ...
    try:
        ...
        assert reupgraded == ("0049_cleanup_terminal_counts", None, "UNKNOWN")
    finally:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            admin.execute(
                sql.SQL("DROP DATABASE IF EXISTS {} WITH (FORCE)").format(
...
def test_candidate4_applied_0025_valid_tx05_time_proof_advances_to_0026(
    ...
    try:
        ...
        assert after == ("0049_cleanup_terminal_counts",)
    finally:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            admin.execute(
                sql.SQL("DROP DATABASE IF EXISTS {} WITH (FORCE)").format(
