...
def test_formation_off_shadow_canary_and_restart_preserve_raw_authority(
    ...
    assert evidence_id in canary.json["evidence_refs"]
...
def test_l1_merges_fts_vector_and_persists_payload_free_append_only_trace(
    ...
    assert access_trace["terminal_stage"] == "CANONICAL_GATE"
...
def test_query_first_memory_resolve_is_task_free_typed_and_governed(
    ...
    assert hit.json["context_receipt"]["schema_version"] == "context-receipt-v0.2"
    ...
    assert "task" not in json.dumps(hit.json).lower()
    ...
    assert absent.json["memory_intent"] == "POSSIBLE"
...
def test_memory_resolve_hard_partitions_entity_type_scope_and_time_before_vector(
    ...
    assert progressive["candidate_counts"]["vector"] <= access_plan["candidate_cap"]
...
@pytest.mark.integration
def test_dg11_compare_operator_consumes_only_canonical_gated_operands(
    retrieval_runtime,
) -> None:  # type: ignore[no-untyped-def]
    settings, app, worker_database = retrieval_runtime
    client = app.test_client()
    ...
    assert response.json["derived_result"]["status"] == "OK"
    assert response.json["derived_result"]["value"] == 6.0
    assert response.json["derived_result"]["hidden_model_calls"] == 0
    assert response.json["derived_result"]["canonical_mutation"] is False
    assert {item["claim_version_id"] for item in response.json["derived_result"]["operands"]} == {
