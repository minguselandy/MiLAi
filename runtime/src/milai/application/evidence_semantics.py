...
_EVENT_SIGNAL = re.compile(
    ...
    r"participat(?:e|ed|ing)|plant(?:ed|ing)?|prepar(?:e|ed|ing)|read|"
    r"repair(?:ed|ing)?|scheduled|shared|started|tried|used|visited|"
    r"volunteer(?:ed|ing)?|went|"
...
def project_evidence_spans(
    ...
    for item in evidence:
        ...
        if not all(
            isinstance(value, str) and value for value in (content, evidence_id, source_ref)
        ):
            continue
        ...
        source_identity = structured_evidence_identity(item, source_ref)
        if source_identity is None:
            continue
        speaker, speaker_source = structured_evidence_speaker(item)
        for start, end in _source_offsets(content):
            text = content[start:end]
            span_id = _identity("span", [evidence_id, start, end, text])
            span = EvidenceSpan(
                span_id=span_id,
                ...
                provenance={
                    ...
                    "source_content_hash": item.get("content_hash"),
...
def _interpret_evidence_spans(
    ...
    *,
    ...
    suppressed = 0
    for span in spans:
        entities = sorted(_semantic_terms(_WORD.findall(span.text)))[:32]
        date_ranges = _date_match_ranges(span.text)
        explicit_time = _explicit_event_time(span.text, span.source_timestamp)
        ...
        if explicit_time is not None or _EVENT_SIGNAL.search(span.text) is not None:
        ...
        if _PREFERENCE.search(span.text) is not None:
        ...
        if has_explicit_current_intent(span.text) and span.speaker == "user":
    ...
    for requirement in requirements:
        for interpretation in interpretations:
            ...
            if compatibility_profile == "dg22-v0.2":
                predicate_status = _predicate_compatibility(requirement, interpretation)
...
def _entity_compatibility(
    ...
    *,
    ...
    required = _semantic_terms(requirement.entity_constraints)
    if not required:
...
def _explicit_event_time(
    ...
    weekend = _LAST_WEEKEND.search(text)
    ...
    weekday = _RELATIVE_WEEKDAY.search(text)
...
def _date_match_ranges(text: str) -> list[tuple[int, int]]:
    return [
        match.span()
        for pattern in (
            _ISO_DATE,
