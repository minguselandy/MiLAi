...
def _matched_binding_options(
    ...
    for binding in bindings:
        if binding.status != "MATCH" or binding.requirement_id not in options:
            continue
...
def execute_binding_backed_query_operator(
    ...
    concepts.  The legacy executor may inspect the former for operators that
    have not migrated yet; a between-events temporal distance, however, has a
    complete typed value contract and therefore must consume exactly one
    independently resolved event-time binding per required slot.
    """
    ...
    if not supported:
        return execute_query_operator(plan, results)
    ...
    for binding in bindings:
        if binding.status != "MATCH" or binding.requirement_id not in temporal_options:
            continue
        interpretation = interpretation_by_id.get(binding.interpretation_id)
        if (
            interpretation is None
            or interpretation.kind != "EVENT"
            or interpretation.event_time is None
            or interpretation.event_time.start is None
        ):
            continue
        ...
        if item is None:
            continue
        temporal_options[binding.requirement_id].append((span, interpretation, item))
...
def execute_query_operator(
    ...
    if operator == "TEMPORAL_DISTANCE":
        ...
        if mode == "from_reference":
            ...
            distance_value = _distance_value(anchor_event.at, plan.time_reference, unit)
            return _base_success(
                plan,
                operator,
                [anchor_event.item],
