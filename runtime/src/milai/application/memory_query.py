...
from milai.application.query_ir_compat import synthesize_memory_query_ir_v02
...
_COUNT = re.compile(r"\b(?:how many|count(?: of)?|number of)\b", re.IGNORECASE)
...
_RANGE = re.compile(
    r"\b(?:in|during|over|within|for)?\s*(?:the\s+)?(?:past|last)\s+"
...
_CN_COUNT = re.compile(r"(?:多少次|有多少(?:个|件|次|位|只|辆)?|数量(?:是|为)?多少)")
...
class MemoryQueryCompiler:
    ...
    def compile(
        ...
        *,
        ...
        source_policy, semantic_query = _extract_source_policy(query)
        semantic_spec = self._compile_semantic_spec(
            semantic_query, reference_time=reference_time
        )
        translated = synthesize_memory_query_ir_v02(
            semantic_spec,
            query=semantic_query,
            scope=scope,
            compiler_version=self.VERSION,
            reason_prefix="NATIVE_V02",
        )
        if source_policy is not None:
            translated = _apply_source_policy(translated, source_policy)
        if self._preference_current_intent_enabled:
            translated = _expand_preference_current_intent(translated, semantic_query)
        compiler_version = self.VERSION
        if any(requirement.slot_id == "CURRENT_INTENT" for requirement in translated.requirements):
            compiler_version = f"{compiler_version}+dg21-current-intent-v1"
        traced = translated.model_copy(
            update={
                "planner_trace": translated.planner_trace.model_copy(
                    update={"compiler_version": compiler_version}
                )
            }
        )
        return traced.model_copy(
            update={"lexical_cues": compile_lexical_cue_sets(traced.requirements)}
        )
    ...
    def _compile_semantic_spec(
        ...
        if _COUNT.search(text) and _DISTANCE.search(text) is None:
...
def _distance_anchors(query: str) -> list[list[str]]:
    for pattern in (_BETWEEN, _BEFORE_DID, _SINCE_WHEN, _FROM_TO):
...
def _range_constraint(query: str, reference: datetime) -> TemporalConstraint | None:
    ...
    if matched is not None:
        count = _count(matched.group("count"))
        unit = matched.group("unit").casefold().rstrip("s")
        return TemporalConstraint(
            reference_time=reference,
            start=_shift(reference, count, unit),
            end=reference,
