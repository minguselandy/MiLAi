...
from milai.domain.reader_evidence_plan import AcceptedBindingSpan, DecisionSnapshot
...
class RetrievalService:
    ...
    def retrieve(
        ...
        *,
        ...
        acquisition_execution: EvidenceAcquisitionExecution | None = None
        ...
        try:
                    ...
                    if self._deterministic_recovery_enabled:
                        ...
                        recovery = (
                            ...
                            if terminal_selection is not None
                                ...
                                policy=recovery_policy,
                                baseline_execution=acquisition_execution,
                                mode="PRODUCT",
                                type_directed_semantics=(self._type_directed_semantics_enabled),
                                execution_policy=(
                                    self._acquisition_execution_policy
            ...
            if assembled is None:
                ...
                derived_result = (
                    ...
                    if evidence_composition is not None
                        ...
                        "query_operator_ms", execute_query_operator, plan, assembled[2]
                    )
                )
            ...
            trace_id = stages.call(
                ...
                RetrievalTraceCommand(
                    ...
                    query_plan=cast(dict[str, object], plan.model_dump(mode="json")),
...
def _merge_candidates(
    ...
    *,
    ...
    ranked = [
        RetrievalCandidate(version_id, score, "canonical") for version_id, score in scores.items()
    ]
...
def _acquisition_reference_material(
    ...
    *,
    execution: EvidenceAcquisitionExecution | None = None,
...
def _rank_evidence_turns(
    ...
    return rank_evidence_turns(evidence_results, query)
...
def _rerank_by_reference(
    ...
    if target is not None:
        # Relative-point queries bind time before semantics: a concrete source
        # sentence such as "I got a smoker today" may share no literal noun with
        # "kitchen appliance".  Date proximity builds the bounded pool and the
        # existing cross-encoder supplies the semantic ordering in one batch.
        timed_by_event = [
            (
                index,
...
def _decision_snapshot(
    *,
    ...
    acquisition_execution: EvidenceAcquisitionExecution | None,
...
def _decision_accepted_evidence_ids(
    *,
    ...
    accepted_ids = (
        ...
        if acquisition_state is not None and acquisition_state.accepted_evidence_refs
...
def _accepted_binding_spans(
    execution: EvidenceAcquisitionExecution | None,
    *,
