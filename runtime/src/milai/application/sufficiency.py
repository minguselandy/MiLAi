...
_UNEXECUTED_COUNT = re.compile(
    r"\b(?:how many|count(?: of)?|number of)\b", re.IGNORECASE
)
...
def decide_sufficiency(
    ...
    *,
    ...
    if memory_query_ir is not None and memory_query_ir.mode == "AMBIGUOUS":
        return (
            SufficiencyDecision(
                status="UNSATISFIED",
                missing_slots=[
                    requirement.slot_id
                    for requirement in memory_query_ir.requirements
                    if requirement.required
                ],
                stop_reason="QUERY_AMBIGUOUS",
            ),
            "MEMORY_QUERY_IR_AMBIGUOUS",
        )
    ...
    if memory_query_ir is not None and operator_family != "LOOKUP":
        missing = [
            requirement.slot_id
            for requirement in memory_query_ir.requirements
            if requirement.required
        ...
        unbounded = (
            memory_query_ir.completeness == "ALL_MATCHES_IN_RANGE"
    ...
    implicit_slots = _unexecuted_requirement_slots(request.query or "")
    if implicit_slots:
        return (
            SufficiencyDecision(
...
def _unexecuted_requirement_slots(query: str) -> list[str]:
    if _UNEXECUTED_COUNT.search(query):
