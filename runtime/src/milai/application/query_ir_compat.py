...
def synthesize_memory_query_ir_v02(
    ...
    *,
    ...
    steps = _steps(operator_family, requirements, normalized_temporal)
    temporal_spans = (
...
def execution_operator_from_ir(
    ...
    if family == "TEMPORAL_DISTANCE":
        mode = next(
            (
                predicate.removeprefix("distance_mode:")
                for requirement in query_ir.requirements
                for predicate in requirement.predicate_constraints
                if predicate.startswith("distance_mode:")
            ),
            "between_events",
        )
        anchors = _requirement_anchors(query_ir)
        unit = next(
            (
                predicate.removeprefix("distance_unit:")
                for requirement in query_ir.requirements
                for predicate in requirement.predicate_constraints
        ...
        return "TEMPORAL_DISTANCE", {
            **common,
            "operand_type": "temporal_event",
            "required_operand_count": 1 if mode == "from_reference" else 2,
            "distance_mode": mode,
            "distance_unit": unit,
            "date_boundary": "exclusive",
    ...
    if operator_family in {
        ...
        steps.append(
            MemoryQueryStep(
                ...
                constraints={"join_validation": "TYPE_ENTITY_UNIT_TIME_EPISODE"},
            )
        )
    if operator_family == "COUNT":
        steps.append(
    ...
    if reduction is not None:
        steps.append(
            MemoryQueryStep(
                ...
                outputs=["answer"],
                constraints={"operation": reduction},
            )
        )
