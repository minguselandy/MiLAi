...
> 本文为 Phase 1 文本个人记忆生命周期的统一完成合同。它在激活前不替代 `MILA-ML-MASTER@1.6`，不授权立即执行代码、数据、模型、数据库或产品开关操作。用户已预先授权：Program 激活且 C0–C3 开发门通过后，必须执行一次完整 LongMemEval 外部验证；这不等于授权 formal holdout 或 `DEFAULT_ON`。
...
LongMemEval full run:          REQUIRED AFTER ACTIVATION + C0-C3 PASS
